...
VÁLTOZÁS a v1-hez képest: NINCS többé Google Sheet függőség.
  ...
  - E-mail:     VÁLTOZATLAN — Apps Script Web App Gmail-proxy (sendEmail,
                PDFquotationSENDdealOWNER), mert a Service Account tiltott.
...
Környezeti változók:
    ...
    WEBAPP_URL               – Apps Script Web App URL (csak email-küldéshez!)
    WEBAPP_SECRET            – Apps Script közös titok
    TOKEN_SECRET             – token-aláíró titok (ha nincs, WEBAPP_SECRET)
...
WEBAPP_URL           = os.environ.get("WEBAPP_URL", "")      # csak email!
WEBAPP_SECRET        = os.environ.get("WEBAPP_SECRET", "")
BASE_URL             = "https://" + os.environ.get("RAILWAY_PUBLIC_DOMAIN", "")
TOKEN_SECRET         = os.environ.get("TOKEN_SECRET", "") or WEBAPP_SECRET
...
def deal_ertesites_rogzit(deal_id: int, token: str, cimzett: str):
    ...
    url = f"{BASE_URL}/visszajelzes/{token}"
    tartalom = (
...
def visszajelzes_note_rogzit(deal_id: int, idopontok: list, kovetelmenyek: list,
    ...
    """Note a dealre a beküldött visszajelzés tartalmával — a Pipedrive-ban is látszik."""
    ...
    sorok.append(f"Beküldve: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}")
...
def deal_adatok_osszerak(deal_id: int, innonest_kell: bool = True) -> dict | None:
    ...
    Az Innonest-scrape (tételek) csak az email-kiküldéshez kell — az oldal
    rendereléséhez innonest_kell=False, így gyors marad.
    """
...
# ─────────────────────────────────────────────
# EMAIL KÜLDÉS – Apps Script-en keresztül (VÁLTOZATLAN)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# PDF ÁRAJÁNLAT EMAIL – deal üzletfelelősének, csatolmánnyal (VÁLTOZATLAN)
...
def visszajelzes_oldal(token):
    ...
    try:
        adatok = deal_adatok_osszerak(deal_id, innonest_kell=False)
        if not adatok:
            return "<h2 style='font-family:sans-serif;padding:40px;color:#c00;'>Ez a link már nem érvényes.</h2>", 404
...
def visszajelzes_submit():
    ...
    # Dupla submit védelem (best effort, memóriában)
...
def register_pipedrive_routes(app):
    ...
    log.info("[PIPEDRIVE] Végpontok regisztrálva: /pipedrive-webhook, /visszajelzes/<token>, /visszajelzes-submit (v2, Sheet-mentes)")
