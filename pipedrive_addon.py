...
def deal_adatok_osszerak(deal_id: int) -> dict | None:
    ...
    return {
        ...
        # Innonest tételek – a meglévő server.py get_arajanlat_reszletek() tölti ki
        # Ezeket a webhook handler tölti ki külön Playwright hívással
        "tetelek":                [],
        "netto_osszeg":           _szam_fmt(deal.get("value", 0)),
        "fizetesi_feltetelek":    "–",
        "ervenyes_ig":            "–",
    }
...
def pipedrive_webhook():
    """Pipedrive hívja, ha egy deal won-ra változik."""
    adat = request.get_json(silent=True)
    if not adat:
        return jsonify({"ok": False, "hiba": "Üres payload"}), 400

    jelenlegi = adat.get("current", {})
    elozetes  = adat.get("previous", {})

    if jelenlegi.get("status") != "won" or elozetes.get("status") == "won":
        return jsonify({"ok": True, "info": "Nem won változás"}), 200

    deal_id = jelenlegi.get("id")
    if not deal_id:
        return jsonify({"ok": False, "hiba": "Nincs deal ID"}), 400

    log.info(f"[WEBHOOK] Deal megnyerve: {deal_id}")
    ...
    try:
        ...
        email_kuld(adatok["kapcsolattarto_email"], targy, email_html)
    except Exception as e:
