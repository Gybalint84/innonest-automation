...
# "Összesen hány nap a kivitelezés" Pipedrive deal egyedi mező API-kulcsa
PIPEDRIVE_NAPOK_FIELD = os.environ.get("PIPEDRIVE_NAPOK_FIELD_KEY", "cdf639e8db9018be9f880366a03a17aee38284a3")
...
def _innonest_adatok_leker(bid: str) -> dict:
    ...
    Az innonest_core.py innonest_adatok_leker() függvényét hívja.
    """
    try:
...
def _kivitelezes_napok_fmt(ertek) -> str:
    """
    A 'Összesen hány nap a kivitelezés' mező normalizálása.
    Az érték lehet szám (pl. '5') vagy szöveg (pl. '6-8 munkanap').
    Ha már tartalmazza a 'nap' szót, változatlanul hagyjuk;
    egyébként hozzáfűzzük a 'munkanap' szót.
    """
...
def deal_adatok_osszerak(deal_id: int) -> dict | None:
    ...
    # "Összesen hány nap a kivitelezés" egyedi mező
...
def sablon_kitolt(html: str, adatok: dict, token: str = "") -> str:
    ...
    # ── Kivitelezési időtartam blokkok (üres érték esetén nem jelenik meg) ──
...
# ─────────────────────────────────────────────
# Dedikált, önálló függvény az árajánlat PDF generátorhoz (arajanlat_pdf.py).
# SZÁNDÉKOSAN KÜLÖN az email_kuld()-tól, hogy a "Won" automatizáció
# ügyfél-emailjeit semmilyen módosítás ne érintse.
...
# ─────────────────────────────────────────────
# HELYSZÍNI KAPCSOLATTARTÓ – HTML blokk (owner / kivitelező / ügyfél emailekhez)
# ─────────────────────────────────────────────
def site_contact_html_blokk(helyszini_kapcsolattarto: dict, cell_padding: str = "20px 28px") -> str:
    """
    Generálja a helyszíni kapcsolattartó (a kivitelezés alatt kereshető ügyféloldali
    személy) HTML blokkját. Üres/hiányzó adat esetén nem jelenít meg semmit.
    """
...
def kivitelezonkenti_email_html(deal_adatok: dict, idopontok: list,
                                kovetelmenyek: list, megjegyzes: str,
                                kivitelező_nev: str, kivitelező_tetelek: list,
                                helyszini_kapcsolattarto: dict = None) -> str:
...
def visszajelzes_submit():
    ...
    o_html = owner_email_html(deal, idopontok, kovetelmenyek, megjegyzes, kalkulacio, helyszini_kapcsolattarto)
    ...
    if len(kivitelezok) > 1:
        for kiv_nev, kiv_tetelek in kivitelezok.items():
            kiv_html = kivitelezonkenti_email_html(
                ...
                kiv_nev, kiv_tetelek, helyszini_kapcsolattarto
            )
    ...
    u_html = ugyfel_visszaigazolo_html(deal, idopontok, kovetelmenyek, megjegyzes, helyszini_kapcsolattarto)
