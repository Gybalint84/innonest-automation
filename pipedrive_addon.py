...
def arbekero_feladatok_html(tasks: list) -> str:
    ...
    for i, t in enumerate(tasks):
        ...
        html += (
            ...
            f'<td style="padding:12px 10px;border-bottom:1px solid #f0f0f0;text-align:center;width:40px;">'
            f'<span style="display:inline-block;width:24px;height:24px;line-height:24px;border-radius:50%;'
            f'background:#FFD21F;color:#12233b;font-weight:700;font-size:12px;">{i + 1}</span></td>'
            f'<td style="padding:12px 10px;border-bottom:1px solid #f0f0f0;font-weight:600;color:#1a1a1a;font-size:13px;">{t.get("name","")}</td>'
            f'<td style="padding:12px 10px;border-bottom:1px solid #f0f0f0;text-align:right;color:#333;font-size:13px;white-space:nowrap;">{t.get("quantity","")}</td>'
            f'<td style="padding:12px 10px;border-bottom:1px solid #f0f0f0;color:#333;font-size:13px;">{t.get("unit","")}</td>'
            f'</tr>'
        )
    return html
...
def arbekero_reply_mailto(request_id: str, project_location: str, contractor_nev: str, tasks: list) -> str:
    """Az "AJÁNLATOT ADOK" gomb — előre kitöltött válasz e-mailt nyit a
    saját levelezőben, az info@sqm-hungary.hu címre."""
    subject = f"Ajánlat – {request_id or 'SQM projekt'}"
    task_blocks = "\n\n".join(
        f'{i + 1}. {t.get("name","")}\n'
        f'Mennyiség: {t.get("quantity","")} {t.get("unit","")}\n'
        f'Egységár: ____ Ft/{t.get("unit","")}\n'
        f'Összesen: ____ Ft + ÁFA'
        for i, t in enumerate(tasks)
    )
    body = (
        f"Kedves SQM Hungary!\n\n"
        f"Azonosító: {request_id or ''}\n"
        f"Projekt: {project_location or ''}\n\n"
        f"{task_blocks}\n\n"
        f"Üdvözlettel:\n{contractor_nev or ''}"
    )
...
def arbekero_kitolt(adatok: dict) -> str:
    ...
    megjegyzes = (adatok.get("megjegyzes") or "").strip()
    tasks = adatok.get("tasks") or []

    megjegyzes_blokk = (
        f"""<div style="background:#fff8e8;border:1px solid #f1d38a;border-radius:8px;padding:18px;margin:20px 0;">
         <p style="margin:0 0 6px;font-size:13px;font-weight:700;color:#a9781a;text-transform:uppercase;letter-spacing:1px;">SQM megjegyzés</p>
    ...
    csere = {
        ...
        "{{SQM_MEGJEGYZES_BLOKK}}": megjegyzes_blokk,
...
def register_pipedrive_routes(app):
    ...
    log.info("[PIPEDRIVE] Végpontok regisztrálva: /pipedrive-webhook, /visszajelzes/<token>, /visszajelzes-submit, /send-alv-email, /send-arbekero-email (v2, Sheet-mentes)")
