...
PIPEDRIVE_VISSZAJELZES_URL_FIELD = os.environ.get("PIPEDRIVE_VISSZAJELZES_URL_FIELD_KEY", "3470b03aa388c791d1a5b11e107714da1a5a086f")  # Visszajelzés oldal URL-je (beküldéskor íródik be)
...
def sablon_kitolt(html: str, adatok: dict, token: str = "", zarolt_adatok: dict | None = None) -> str:
    """
    zarolt_adatok: ha nem None, a visszajelzes_oldal.html "zárolt" (már
    beküldött, nem szerkeszthető) módban jelenik meg — a JS az itt kapott
    JSON-t olvassa ki és tölti vissza az űrlap mezőibe letiltott állapotban.
    A visszajelzes_oldal.html-en kívüli sablonoknál (email) ez a paraméter
    nincs használatban, {{ZAROLVA_JSON}} placeholder ott sincs.
    """
    owner_tel_sor = (
    ...
    # ── Zárolt (beküldött) adatlap JSON-ja a visszajelzes_oldal.html JS-éhez ──
    if zarolt_adatok is not None:
        zarolva_json = json.dumps(zarolt_adatok, ensure_ascii=False)
        zarolva_json = zarolva_json.replace("</", "<\\/")  # ne törje meg a </script>-et
    else:
        zarolva_json = "null"
...
def visszajelzes_oldal(token):
    ...
    try:
        ...
        # ── Zárolás: ha már beküldte, ugyanaz az űrlap jelenik meg, csak
        # letiltott mezőkkel és a beküldött értékekkel kitöltve — a JS
        # (lockForm) a {{ZAROLVA_JSON}}-ből olvassa vissza az adatokat.
        beerkezett = deal_visszajelzes_adatok(deal_id)

        html = sablon_kitolt(sablon_betolt("visszajelzes_oldal.html"), adatok, token,
                             zarolt_adatok=beerkezett)
        return html, 200
    except Exception as e:
...
def visszajelzes_submit():
    ...
    # ── Visszajelzés oldal linkjének beírása a dedikált Pipedrive mezőbe ──
    # (a mező kitöltése egyben jelzi is, hogy az ügyfél már beküldte az
    # adatlapot — az értékesítő innen egy kattintással eléri a zárolt oldalt)
    try:
        visszajelzes_url = f"{BASE_URL}/{token}"
        _pd_post(f"deals/{deal_id}", {PIPEDRIVE_VISSZAJELZES_URL_FIELD: visszajelzes_url})
    except Exception as e:
        log.warning(f"[SUBMIT] Visszajelzés URL mező írása sikertelen: {e}")
...
# ─────────────────────────────────────────────
def _pd_deal_cim(deal_id) -> dict:
    """A dealhez tartozó cím-adatok: cégnév + Kivitelezés helyszíne (deal mező),
    valamint a SZERVEZET SZÉKHELYE irányítószámra, településre és utca/házszámra
    bontva, plusz a szervezet adószáma. A geocode-olt al-mezőket használja
    (address_postal_code / address_route / address_street_number / address_locality);
    ha nincsenek, a teljes cím stringjéből próbálja kibontani (magyar formátum:
    '1117 Budapest, Példa utca 12.')."""
    _ORG_FIELD_ADOSZAM = "f8032f2bfb73caa261e7459ab2224b1a3704a111"  # Org: Adószám (egyedi mező)
    out = {"cegnev": "", "helyszin": "", "iranyitoszam": "", "utca": "", "telepules": "", "adoszam": ""}
    deal = _pd_get(f"deals/{deal_id}") or {}
    out["helyszin"] = str(deal.get(PIPEDRIVE_SITE_ADDRESS_FIELD) or "").strip()
    org_ref = deal.get("org_id")
    org_id = org_ref.get("value") if isinstance(org_ref, dict) else org_ref
    org = (_pd_get(f"organizations/{org_id}") if org_id else {}) or {}
    out["cegnev"] = (org.get("name") or deal.get("org_name") or "").strip()
    out["adoszam"] = str(org.get(_ORG_FIELD_ADOSZAM) or "").strip()

    postal = str(org.get("address_postal_code") or "").strip()
    route  = str(org.get("address_route") or "").strip()
    num    = str(org.get("address_street_number") or "").strip()
    city   = str(org.get("address_locality") or "").strip()
    utca   = (route + " " + num).strip()
    full   = str(org.get("address") or "").strip()

    # Ha az al-mezők hiányoznak (nem geocode-olt cím), a teljes stringből bontjuk.
    if (not postal or not utca or not city) and full:
        m = re.match(r"^\s*(\d{4})\s+([^,]+?)[,\s]+(.+?)(?:,\s*(?:Magyarország|Hungary))?\s*$", full)
        if m:
            postal = postal or m.group(1)
            city   = city or m.group(2).strip()
            utca   = utca or m.group(3).strip().strip(",")

    out["iranyitoszam"] = postal
    out["utca"] = utca
    out["telepules"] = city  # a székhelyből lebontott település
    # Ha a deal 'Kivitelezés helyszíne' üres, a székhely városa a tartalék.
    if not out["helyszin"] and city:
        out["helyszin"] = city
    return out
...
def pipedrive_project():
    ...
    # Cím-adatok (cégnév, helyszín, székhely → irányítószám + település + utca, adószám)
    # — akkor is, ha nincs a dealhez kötött projekt.
    try:
        cim = _pd_deal_cim(deal_id)
    except Exception as e:
        log.warning(f"[PD] cím lekérés hiba (deal {deal_id}): {e}")
        cim = {"cegnev": "", "helyszin": "", "iranyitoszam": "", "utca": "", "telepules": "", "adoszam": ""}

    proj = _pd_project_by_deal(deal_id)
    if not proj:
        return jsonify({"ok": True, "found": False, "dealId": deal_id, **cim})
