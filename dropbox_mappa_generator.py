# -*- coding: utf-8 -*-
"""
Dropbox mappa auto-generátor Pipedrive webhookból + fotó-visszaolvasás.

FOLYAMAT (ÍRÁS, webhook):
...
FOLYAMAT (OLVASÁS, a kalkulátor "Fotók" menüje):
1. A kalkulátor GET /pipedrive-dropbox-photos?deal_id=... vagy ?bid=... hívást küld.
2. A szerver megkeresi a dealt (deal_id vagy BID alapján), kiolvassa a Dropbox-mappalinket.
3. A Dropbox API-val listázza a mappa képfájljait, mindegyikhez ideiglenes (kb. 4 órán
   át érvényes), közvetlenül megnyitható linket ad — mert egy sima megosztott mappalink
   önmagában nem listázható/nem jeleníthető meg közvetlenül a böngészőből.
...
Ezután az endpointok ugyanazon a domainen lesznek elérhetők, ahol a többi
automatizációtok fut, pl.:
    https://<railway-domain>/pipedrive-webhook/dropbox-mappa   (írás, webhook)
    https://<railway-domain>/pipedrive-dropbox-photos          (olvasás, kalkulátor)
...
# A BID szám alapján történő deal-kereséshez a pipedrive_webapp.py már meglévő
# segédfüggvényét használjuk újra (ugyanabban a repóban van), hogy ne kelljen
# ugyanazt a Pipedrive keresési logikát még egyszer megírni.
...
# a "Fotók" menü fotó-listázásához: csak ezekkel a kiterjesztésekkel végződő
# fájlokat vesszük figyelembe, és egy mappában legfeljebb ennyit listázunk
# (válaszidő-védelem, hogy egy nagyon tele mappa se lassítsa be a menüt)
...
# ---------------------------------------------------------------------------
# DROPBOX MŰVELETEK (ÍRÁS: mappa létrehozás + megosztott link)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ENDPOINT REGISZTRÁCIÓ
# ---------------------------------------------------------------------------

def register_dropbox_routes(app):
    """Regisztrálja a Dropbox mappa-generáló webhook végpontot ÉS a "Fotók"
    menü fotó-visszaolvasó végpontját a Flask app-on.
