...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    A Dropzone.js nem globális változó (dzDefined=False), ezért:
    1. BID sor indexét megkeressük
    2. Klikkel triggereljük a Dropzone inicializálást
    3. A nth(bid_index) file inputra töltünk fel
    """
    ...
    initial_count = await page.locator('input[type="file"]').count()
    log.info(f"[PDF] File inputok lapbetöltés után: {initial_count}")

    if initial_count == 0:
        bid_rows = page.locator("tr").filter(has=page.locator('a[href*="worksheets_pdf"]'))
        target_row = bid_rows.nth(bid_index)
        first_td = target_row.locator("td").first
        if await first_td.count() > 0:
            await first_td.click()
            log.info("[PDF] TD kattintás megtörtént")
        else:
            await target_row.click()
            log.info("[PDF] TR kattintás (fallback)")
        await page.wait_for_timeout(2000)
        count_after = await page.locator('input[type="file"]').count()
        log.info(f"[PDF] File inputok TD kattintás után: {count_after}")
        ...
        if count_after == 0:
            ...
            await page.wait_for_timeout(1500)
            count_scroll = await page.locator('input[type="file"]').count()
            log.info(f"[PDF] File inputok görgetés után: {count_scroll}")

    total = await page.locator('input[type="file"]').count()
    if total == 0:
        log.warning("[PDF] Dropzone init sikertelen – upload_csatolmany fallback")
        await upload_csatolmany(page, {"nev": filename, "adat": pdf_b64}, bid)
        return

    idx = bid_index if bid_index < total else 0
    log.info(f"[PDF] File input használata: index {idx} / összesen {total}")
