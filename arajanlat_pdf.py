...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    # Az Innonest Dropzone.js-t használ fájlfeltöltéshez.
    # A rejtett dz-hidden-input elemek NEM a TR-en belül vannak —
    # Dropzone a body szinten hozza létre őket, de UGYANOLYAN SORRENDBEN
    # mint a BID sorok. Ezért a BID sor pozícióját keressük meg a listában,
    # majd a megfelelő indexű file inputot használjuk.
    ...
    bid_index = await page.evaluate(
        ...
        (bid) => {
            const escaped = bid.replace(/-/g, '\\\\-');
            const bidRegex = new RegExp('(^|[^0-9-])' + escaped + '([^0-9]|$)');
            // Minden sor ami tartalmaz worksheets_pdf linket = BID sor
    ...
    if bid_index < 0:
        raise Exception(f"PDF feltöltés: nem találtam a BID sort a listában: {bid}")

    log.info(f"[PDF] BID sor index a listában: {bid_index}")

    # A Dropzone.js csak kattintásra hozza létre a rejtett file inputokat.
    # Megkeressük a helyes BID sort és kattintunk a Dropzone területére (td.td-nopadding).
    bid_rows = page.locator('tr').filter(
        has=page.locator('a[href*="worksheets_pdf"]')
    )
    target_row = bid_rows.nth(bid_index)

    # Kattintás a Dropzone konténer TD-re – ez triggereli a rejtett input létrejöttét
    dropzone_td = target_row.locator('td.td-nopadding').first
    if await dropzone_td.count() > 0:
        await dropzone_td.scroll_into_view_if_needed()
        await dropzone_td.click()
        log.info("[PDF] Dropzone TD kattintva")
    else:
        # Fallback: az első gomb a sorban
        fallback_btn = target_row.locator('button').first
        if await fallback_btn.count() > 0:
            await fallback_btn.click()
            log.info("[PDF] Dropzone fallback gomb kattintva")

    # Várjuk a file input megjelenését (Dropzone aszinkron hozza létre)
    await page.wait_for_timeout(2000)
    total_inputs = await page.locator('input[type="file"]').count()
    log.info(f"[PDF] Dropzone inputok száma: {total_inputs}")

    if total_inputs == 0:
        raise Exception("PDF feltöltés: Dropzone inputok nem jelentek meg kattintás után")

    # A bid_index-edik input a mi sorunkhoz tartozik
    file_input = page.locator('input[type="file"]').nth(
        bid_index if bid_index < total_inputs else 0
    )
    if await file_input.count() == 0:
        raise Exception(f"PDF feltöltés: nem találtam file inputot a {bid_index}. indexen ({bid})")
