...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    # Megvárjuk hogy a Dropzone.js inicializálja a rejtett file inputokat
    # (aszinkron töltődnek be, a page load után)
    try:
        await page.wait_for_selector(
            'input[type="file"]', state="attached", timeout=8000
        )
    except Exception:
        raise Exception("PDF feltöltés: Dropzone inputok nem jelentek meg (timeout 8s)")

    file_input = page.locator('input[type="file"]').nth(bid_index)
