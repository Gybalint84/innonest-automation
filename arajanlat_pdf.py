...
def tetel_sor(item: dict) -> str:
    ...
    Az Innonest tétel-listájában szabad szöveges "szövegblokk" sor is
    felvehető (nincs név/mennyiség/ár, csak egy nagy szövegdoboz) — ezt az
    `is_text_block` jelöli, és itt a normál termék-rács (név/mennyiség/ár
    fejléc) nélkül, önálló bekezdésként jelenítjük meg.
    """
    if item.get("is_text_block"):
        szoveg = (item.get("leiras") or "").strip()
        if not szoveg:
            return ""
        return (
            '<div style="break-inside:avoid;padding:9px 8px 13px;'
            'font-size:10px;color:#444;line-height:1.6;white-space:pre-line;">'
            f'{_esc(szoveg)}</div>'
        )
...
def build_arajanlat_html(adatok: dict, sablon: str = "a") -> str:
    ...
    # A szövegblokk-tételeknek nincs mennyisége/egysége, ezért kihagyjuk ebből
    # a számításból (különben egyetlen szövegblokk is letiltaná a sort).
    netto_egysegar_sor = ""
    arazott_items = [it for it in items if not it.get("is_text_block")]
    egysegek = set((it.get("egyseg") or "").strip().lower() for it in arazott_items)
    if egysegek and egysegek.issubset({"m²", "m2"}):
        try:
            mennyisegek = set()
            for it in arazott_items:
            ...
            if len(mennyisegek) == 1:
                osszeg = sum(_to_float(it.get("egysegar", 0)) for it in arazott_items)
                netto_egysegar_sor = (
...
async def _scrape_arajanlat(page, bid: str) -> dict:
    ...
    # ── Tételek + leírások ──
    # FONTOS (2026-08): az Innonest tétel-listájában a termék-sorok mellett
    # szabad szöveges "szövegblokk" sor is felvehető (nincs név/mennyiség/ár
    # mezője, csak egy nagy textarea). Korábban ezeket a sorokat a `if (!nev)
    # return;` feltétel csendben KIHAGYTA, ezért a szövegblokk sosem került be
    # a PDF-be, holott az Innonest felületén látszott. Most: ha nincs terméknév,
    # de van tartalommal töltött textarea a sorban, azt külön "szöveg" típusú
    # tételként visszaadjuk (name/qty/price nélkül) — a PDF ezt teljes szélességű
    # bekezdésként jeleníti meg (lásd tetel_sor()).
    tetelek = await page.evaluate(
        ...
        () => {
            ...
            rows.forEach(tr => {
                ...
                // kapják — ezeket a PDF-be sem szabad beletenni, mert a
                // beszerzési ár nem az ügyfélnek való.
                ...
                if (!nev) {
                    // Nincs terméknév — lehet, hogy ez egy szabad szöveges
                    // "szövegblokk" sor. Ha van benne szöveggel kitöltött
                    // textarea, azt önálló szöveg-tételként vesszük fel.
                    const taInput = tr.querySelector('textarea');
                    const szoveg = taInput ? taInput.value.trim() : '';
                    if (szoveg) {
                        out.push({
                            megnevezes: '', mennyiseg: '', egyseg: '',
                            egysegar: '', osszesen: '',
                            leiras: szoveg,
                            is_text_block: true,
                        });
            ...
            return { items: out, hidden_count };
        }
        """
    )
    hidden_count = tetelek.get("hidden_count", 0)
    tetelek = tetelek.get("items", [])
    eredmeny["tetelek"] = tetelek
    log.info(f"[PDF] {len(tetelek)} tétel kiolvasva")
    if hidden_count:
        log.info(f"[PDF] {hidden_count} elrejtett (hideElementsRow) sor kihagyva a PDF-ből")
    for i, t in enumerate(tetelek):
        if t.get("is_text_block"):
            log.info(f"[PDF]   [{i+1}] SZÖVEGBLOKK ({len(t['leiras'])} karakter)")
        else:
            log.info(
