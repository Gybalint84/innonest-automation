...
async def _scrape_arajanlat(page, bid: str) -> dict:
    ...
    # A valódi Innonest mezőneveket a korábbi debug logból ismerjük:
    #   crDate     → keltezés  (input[name=crDate])
    #   expiration → érvényes  (input[name=expiration])
    #   paymentMethod → fizetési mód (select[name=paymentMethod] – szöveget, nem ID-t olvasunk)
...
async def _set_innonest_status_elkuldve(page, bid: str):
    ...
    # Modálban az 'Elküldve' opciót választjuk ki.
    # Az Innonest egy <select> elemet használ a státusz változtatáshoz,
    # ezért select_option()-t kell használni, nem .click()-et.
    changed = await page.evaluate("""
        () => {
            const selects = document.querySelectorAll('select');
            for (const sel of selects) {
                const opts = [...sel.options];
                const opt = opts.find(o => o.text.trim() === 'Elküldve');
                if (opt) {
                    sel.value = opt.value;
    ...
    """)
    log.info(f"[PDF] Státusz select JS eredmény: {changed}")

    if not changed.get("ok"):
        # Fallback: Playwright select_option
        status_select = page.locator("select").filter(
            has=page.locator("option", has_text="Elküldve")
        ).first
        if await status_select.count() > 0:
            await status_select.select_option(label="Elküldve")
            log.info("[PDF] Státusz Playwright select_option-nal beállítva")
        else:
            log.warning("[PDF] Státusz select nem található")
            return

    await page.wait_for_timeout(500)

    # Mentés/megerősítés ha szükséges (submit gomb keresés)
    submit = page.locator("button[type=submit], input[type=submit]").first
    if await submit.count() > 0:
        await submit.click()
        await page.wait_for_timeout(1000)
        log.info("[PDF] Státusz mentés gomb kattintva")
