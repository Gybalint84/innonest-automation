...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    # DEBUG: kiírjuk a sor HTML-jét, hogy lássuk milyen gombok vannak benne
    try:
        row_html = await bid_row.inner_html()
        log.info(f"[PDF-DEBUG] BID sor HTML (első 800 kar): {row_html[:800]}")
    except Exception as e:
        log.warning(f"[PDF-DEBUG] HTML log hiba: {e}")
