...
async def _scrape_arajanlat(page, bid: str) -> dict:
    ...
    # ── Keltezés / Érvényesség / Fizetési feltétel ──────────────────────────
    # Form inputok közvetlen értéke megbízhatóbb mint a innerText regex,
    # mert date picker és select mezők értéke nem mindig jelenik meg a szövegben.

    form_vals = await page.evaluate("""
        () => {
            const get = (names) => {
                for (const n of names) {
                    // input/select name egyezés
                    const el = document.querySelector(
                        `input[name="${n}"], select[name="${n}"], textarea[name="${n}"]`
                    );
                    if (el && el.value && el.value.trim()) return el.value.trim();
                    // label szöveg alapú keresés
                    const labels = [...document.querySelectorAll('label')];
                    for (const lbl of labels) {
                        if (lbl.innerText && lbl.innerText.match(new RegExp(n, 'i'))) {
            ...
            // innerText-ből is próbálunk – ha az input nem található
            const text = document.body ? document.body.innerText : '';
            
            const erv_input = get(['bid_ervenyes','ervenyes','validTo','valid_to','erv_datum']);
            const fiz_input = get(['bid_fizetes','fizetesi_feltetel','payment_term',
                                   'fizetes_mod','payment_condition','paymentTerms']);

            const kel_input = get(['bid_keltezese','keltezés','keltez','issueDate','issue_date','keltezes']);

            // Regex fallback a szövegből
            const kel_m  = text.match(/[Kk]eltez[éeÉE][^\\n]{0,15}?(\\d{4}-\\d{2}-\\d{2})/);
            const erv_m = text.match(/[Éé]rv[eé]nyes[^\\n]{0,20}?(\\d{4}-\\d{2}-\\d{2})/);
            const fiz_m = text.match(/[Ff]izet[eé]si feltétel[ek]*[:\\s]+([^\\n]{3,80})/);

            return {
                keltezes:          kel_input  || (kel_m  ? kel_m[1]  : ''),
                ervenyes:          erv_input || (erv_m ? erv_m[1] : ''),
                fizetesi_feltetel: fiz_input || (fiz_m ? fiz_m[1].trim() : ''),
                // Debug: összes input amit talál a form
                all_inputs: [...document.querySelectorAll('input, select')]
                    .filter(el => el.name && el.value)
                    .map(el => ({name: el.name, value: el.value.substring(0, 50)}))
                    .slice(0, 30)
            };
        }
    """)
    ...
    log.info(f"[PDF] Elérhető form inputok: {form_vals.get('all_inputs', [])}")
...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    """
    PDF feltöltése az Innonestbe a helyes BID-hez.

    Tanulság az eddigi tesztelésből:
    - A Dropzone.js nem globális változó → instances nem érhető el
    - TD/görgetés klikk nem triggereli a Dropzone-t
    - Minden kattintás CSAK az adott sor Dropzone-ját inicializálja (1 input)
    - Az outermost container kattintás MINDIG az 1. sor inputját nyitja

    Megoldás:
    1. Megkeressük a helyes BID sor indexét
    2. ABBAN a sorban keressük a triggert (nem az outermost-ban)
    3. Rákattintunk → 1 file input jelenik meg (az adott sor Dropzone-ja)
    4. .first-et használunk (csak 1 input van)
    """
    ...
    # A SPECIFIKUS BID sor triggere (nem az outermost container első eleme)
    ...
    # Várjuk a Dropzone inputot (1 db fog megjelenni – az adott sor-é)
    await page.wait_for_timeout(1500)
    ...
    # .first – mivel csak 1 input van (az adott sor Dropzone-ja)
...
def _text_to_email_html(text: str) -> str:
    """Sima szöveget (sortörésekkel) HTML email törzzsé alakít."""
    html_text = (
        text
        .replace("&", "&amp;")
        .replace("<", "&lt;")
...
# ══════════════════════════════════════════════════════════════════════════════
#
# 1. fázis (_full_pipeline): scraping + PDF render → status="pdf_ready" (vár)
#    A PDF azonnal letölthető, az üzenet szerkeszthető.
# 2. fázis (_run_phase2): Innonest feltöltés + email → status="done"
#    A "Feltöltés és küldés" gomb váltja ki.
...
async def _full_pipeline(bid: str, job_id: str):
    ...
    # PDF kész — megállunk, várjuk a felhasználói jóváhagyást
    _set_job(
        job_id,
        status="pdf_ready",
        ...
        pdf_path=pdf_path,
        _adatok=adatok,  # 2. fázishoz tároljuk
    )


async def _set_innonest_status_elkuldve(page, bid: str):
    """
    A BID státuszát 'Piszkozat'-ról 'Elküldve'-re állítja.
    Az Innonest /bids listán a státusz gombra kattint, majd a modálban 'Elküldve'-t választ.
    """
    await page.goto("https://app.innonest.hu/bids", wait_until="networkidle")
    await page.wait_for_timeout(1500)

    # BID sorában a státusz cellára (Piszkozat) kattintunk
    bid_rows = page.locator("tr").filter(
        has=page.locator('a[href*="worksheets_pdf"]')
    )
    bid_index = await page.evaluate(
    ...
    if bid_index < 0:
        log.warning(f"[PDF] Státusz: nem találtam a BID sort: {bid}")
        return

    target_row = bid_rows.nth(bid_index)

    # Státusz gomb a sorban (szövege "Piszkozat", "Elkészült" stb.)
    status_btn = target_row.locator(
        "text=Piszkozat, text=Elkészült, text=Elküldve, text=Elfogadott, text=Elutasított"
    ).first
    if await status_btn.count() == 0:
        # Fallback: az utolsó TD-ben keresünk gombot/linket
        status_btn = target_row.locator("td").last.locator("button, a, span").first

    if await status_btn.count() == 0:
        log.warning(f"[PDF] Státusz: nem találtam a státusz gombot a sorban: {bid}")
        return

    await status_btn.click()
    log.info(f"[PDF] Státusz gomb kattintva ({bid})")
    await page.wait_for_timeout(1000)

    # Modálban az 'Elküldve' opcióra kattintunk
    elkuldve_btn = page.locator("text=Elküldve").first
    if await elkuldve_btn.count() == 0:
        log.warning("[PDF] Státusz: 'Elküldve' opció nem található a modálban")
        # Bezárjuk a modált
        zaras = page.locator("text=Ablak bezárása").first
        if await zaras.count() > 0:
            await zaras.click()
        return

    await elkuldve_btn.click()
    await page.wait_for_timeout(1000)
...
def register_pdf_routes(app):
    ...
    def pdf_tool():
        if not _pdf_tool_auth_ok(request):
            # Bejelentkezési oldal mutatása (a tool stílusával)
...
<script>
  ...
  if (p.get('err')) {
    ...
    def pdf_tool_login():
        # Rate limiting: IP-nként max 5 próbálkozás 10 percen belül
        ip = request.headers.get("X-Forwarded-For", request.remote_addr or "").split(",")[0].strip()
        now = __import__("time").time()
        attempts = _login_attempts.get(ip, [])
        attempts = [t for t in attempts if now - t < 600]  # utóbbi 10 perc
        if len(attempts) >= 5:
        ...
        if PDF_TOOL_SECRET and pw == PDF_TOOL_SECRET:
            _login_attempts.pop(ip, None)  # sikeres login: töröljük a számlálót
    ...
    def pdf_download(job_id):
        ...
        return Response(
            ...
            headers={"Content-Disposition": f'inline; filename="{job.get("bid_szam","arajanlat")}.pdf"'},
        )

    log.info("[PDF] Végpontok regisztrálva: /pdf-tool, /generate-arajanlat-pdf, /confirm-arajanlat-pdf/<id>, /pdf-status/<id>, /pdf-download/<id>")
