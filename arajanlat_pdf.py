...
def _pdf_filename(bid: str, ugyfel_nev: str = "") -> str:
    ...
    if ugyfel_nev:
        ...
        # Csak betűk, számok és kötőjelek maradhatnak a fájlnévben
...
# ══════════════════════════════════════════════════════════════════════════════
# ALAPÉRTELMEZETT EMAIL ÜZENET GENERÁLÁS
# ══════════════════════════════════════════════════════════════════════════════

def _generate_default_message(adatok: dict) -> str:
    """
    Az értékesítőnek küldött email alapértelmezett szövegét generálja
    a BID adataiból. A felhasználó szerkesztheti a küldés előtt.
    """
    bid_szam   = adatok.get("bid_szam", "")
    ugyfel_nev = adatok.get("ugyfel_nev", "")
    targya     = adatok.get("targya", "")
    netto      = adatok.get("netto_osszesen", 0)

    netto_str = fmt_huf(netto) if netto else ""

    lines = ["Elkészült egy árajánlat PDF."]
    lines.append("")
    if bid_szam:
        lines.append(
            f"Csatolva találod a {bid_szam} árajánlat PDF verzióját"
            f" — a fájl az Innonestben is csatolásra kerül a BID-hez."
        )
    if ugyfel_nev:
        lines.append(f"\nÜgyfél: {ugyfel_nev}")
    if targya:
        lines.append(f"Tárgy: {targya}")
    if netto_str:
        lines.append(f"Nettó összesen: {netto_str}")
...
async def _set_innonest_status_elkuldve(page, bid: str):
    ...
    A modal-alapú megközelítés (list oldal + kattintás) megbízhatatlan volt,
    mert az opciók <option> elemek egy <select>-ben, ami nem kattintható.

    Ehelyett: a szerkesztő oldalon a status <select name="status"> mezőt
    közvetlenül Elküldve-re állítjuk és mentjük.
    A debug logból ismerjük a pontos mezőneveket.
    """
    # Szerkesztő URL megkeresése
    edit_url = await _find_bid_edit_url(page, bid)
    await page.goto(edit_url, wait_until="networkidle")
    await page.wait_for_timeout(1000)

    # Státusz select beállítása (name="status", Elküldve label)
    ...
    # Mentés – az edit oldal submit gombjára kattintunk
