...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    A Dropzone.js nem globális változóként töltődik be, TD/görgetés klikkel
    sem inicializálható. Az upload_csatolmany-ban lévő "outermost ancestor
    first button" kattintás viszont triggereli az összes Dropzone inputot.

    Megoldás: ugyanazt a klikket alkalmazzuk, de .first helyett .nth(bid_index)-et
    használunk → a helyes BID file inputjára töltünk.
    """
    ...
    # Ugyanaz a trigger mint upload_csatolmany-ban:
    # outermost ancestor first button/link → triggereli az összes Dropzone inputot
    bid_lok = page.locator(f"text={bid}").first
    if await bid_lok.count() > 0:
        sor = bid_lok.locator(
            "xpath=ancestor::tr | ancestor::li | ancestor::div[@class]"
        ).first
        trigger = sor.locator("button, a").filter(
            has=page.locator('[class*="attach"], [class*="paper"], [title*="satolm"]')
        ).first
        if await trigger.count() == 0:
            trigger = sor.locator("button:first-child, a:first-child").first

        if await trigger.count() > 0:
            await trigger.scroll_into_view_if_needed()
            await trigger.click()
            log.info("[PDF] Dropzone trigger kattintás megtörtént")
            await page.wait_for_timeout(1500)

    total = await page.locator('input[type="file"]').count()
    log.info(f"[PDF] File inputok száma kattintás után: {total}")

    if total == 0:
        raise Exception("PDF feltöltés: Dropzone inputok nem jelennek meg kattintás után")

    idx = bid_index if bid_index < total else 0
    log.info(f"[PDF] File input: index {idx} / összesen {total}")
    file_input = page.locator('input[type="file"]').nth(idx)
    await file_input.set_input_files(pdf_path)
    await page.wait_for_timeout(4000)
    log.info(f"[PDF] Feltöltve Innonestbe ({bid}), index {idx}")
