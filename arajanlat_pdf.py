...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    # Ez egy rejtett <input type="file" class="dz-hidden-input"> elemet hoz létre
    # soronként – nem kell gombra kattintani, Playwright közvetlenül beállíthatja
    # a fájlt a rejtett inputon, és a Dropzone automatikusan feltölti.

    # 1. Megkeressük a helyes BID sort
    bid_cell = page.locator("td").filter(has_text=bid).first
    ...
    # 2. A sorban lévő rejtett Dropzone file inputot keressük
    file_input = bid_row.locator('input[type="file"]').first
    if await file_input.count() == 0:
        raise Exception(f"PDF feltöltés: nem találtam dz-hidden-input mezőt a sorban: {bid}")

    # 3. Fájl közvetlen beállítása – Dropzone automatikusan kezeli a feltöltést
