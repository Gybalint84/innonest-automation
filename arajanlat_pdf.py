...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    Az upload_csatolmany XPath-ancestor bug miatt mindig az első (legújabb) BID-hez tölt —
    ezért itt más stratégiát alkalmazunk:

    1. Megkeressük a BID sor indexét a /bids listán
    2. A Dropzone.js API-ból kinyerjük az adott sor upload URL-jét
    3. A böngésző fetch API-val (session cookie automatikus) közvetlenül POST-olunk
       → nincs szükség gombkattintásra vagy file input keresésre
    """
    ...
    await page.goto("https://app.innonest.hu/bids", wait_until="networkidle")
    await page.wait_for_timeout(3000)  # Dropzone init ideje

    # BID sor indexének meghatározása a listában
    ...
    if bid_index < 0:
        raise Exception(f"PDF feltöltés: nem találtam a BID sort a listán: {bid}")

    log.info(f"[PDF] BID sor index: {bid_index}")

    # Feltöltés a Dropzone fetch API-val – direkt a helyes BID upload végpontjára
    result = await page.evaluate(
        """
        async (args) => {
            // Megvárjuk a Dropzone inicializálódását (max 5 mp, 500 ms-enként)
            let dz = null;
            for (let i = 0; i < 10; i++) {
                if (typeof Dropzone !== 'undefined' &&
                    Dropzone.instances &&
                    Dropzone.instances.length > args.idx) {
                    dz = Dropzone.instances[args.idx];
                    break;
                }
                // Ha még nincs instance, próbáljuk triggerelni az inicializálást
                if (i === 2) document.body.dispatchEvent(new MouseEvent('mousemove'));
                await new Promise(r => setTimeout(r, 500));
            }

            if (!dz) {
                return {
                    error: 'Dropzone nem inicializálódott',
                    idx: args.idx,
                    dzDefined: typeof Dropzone !== 'undefined',
                    instances: typeof Dropzone !== 'undefined' ? Dropzone.instances.length : 0
                };
            }

            const uploadUrl = dz.options ? dz.options.url : null;
            if (!uploadUrl) return {error: 'Nincs upload URL a Dropzone példányban', idx: args.idx};

            // base64 → Blob konverzió
            ...
            try {
                ...
                return {ok: resp.ok, status: resp.status, url: uploadUrl, response: text.substring(0, 300)};
            } catch(e) {
                return {error: 'fetch hiba: ' + e.message, url: uploadUrl};
            }
        }
        """,
        {"idx": bid_index, "b64": pdf_b64, "filename": filename},
    )

    log.info(f"[PDF] Feltöltés eredmény: {result}")

    if result.get("error"):
        raise Exception(f"PDF Innonest feltöltés sikertelen: {result['error']}")
    if not result.get("ok"):
        raise Exception(
            f"PDF Innonest HTTP hiba {result.get('status')}: {result.get('response','')[:100]}"
        )

    log.info(f"[PDF] Feltöltve Innonestbe ({bid}) → {result.get('url')}")
...
async def _run_phase2(job_id: str, custom_message: str):
    """2. fázis: Innonest feltöltés + email a jóváhagyott üzenettel."""
    job = _get_job(job_id)
