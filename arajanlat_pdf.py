...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str):
    ...
    # JS-sel keressük meg a BID sort és kattintunk a csatolmány gombjára.
    #
    # FONTOS: korábban Playwright XPath union-nal kerestük az ancestor sort
    # ("ancestor::tr | ancestor::li | ancestor::div[@class]"), de ennek .first
    # eleme DOKUMENTUM-SORRENDBEN a legfelső (gyökérhez legközelebbi) ancestor-t
    # adja, ami akár az egész táblázat-konténer is lehet. Ebben az esetben a
    # "button:first-child" a táblázat LEGELSŐ sorának (= legújabb árajánlat)
    # csatolmány gombját találta meg, függetlenül a keresett BID-től.
    #
    # JS closest('tr') garantáltan a legközelebbi <tr> ősszülőt adja vissza.
            ...
            for (const td of allTds) {
                if (bidRegex.test(td.innerText.trim())) {
                    ...
                    let btn = row.querySelector(
                        '[class*="attach"], [class*="paper"], [title*="satolm"]'
                    );
                    if (!btn) {
                        btn = row.querySelector('button, a');
                    }
                    if (btn) {
                        btn.click();
                        return true;
    ...
    if not clicked:
        raise Exception(f"PDF feltöltés: nem találtam a BID sort / csatolmány gombot: {bid}")

    # Várakozás a fájl dialog megnyílására – az új Docker image (sima Python + Chromium)
    # lassabb lehet az első futásoknál, ezért aktívan várjuk a file input megjelenését.
    try:
