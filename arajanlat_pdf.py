...
def _esc_multiline(text) -> str:
    """Mint az _esc(), de a SORTÖRÉSEKET is átviszi a HTML-be (<br>).

    2026.08.14-i JAVÍTÁS: az Innonest tétel-leírásai (textarea) valódi
    sortöréseket tartalmaznak - pl. az "Egyéb feltételek" tételnél 27 db -,
    és az Innonest saját PDF-je ezeket meg is tartja. A generátor viszont
    sima _esc()-cel tette a HTML-be, ahol a böngésző a sortöréseket szóközzé
    vonja össze, így a felsorolások egyetlen, olvashatatlan bekezdéssé folytak
    (a felhasználó ezt jelezte az utolsó két tételnél: "Megjegyzés" és
    "Egyéb feltételek"). Ez a függvény az escape UTÁN cseréli a sortöréseket
    <br>-re, tehát nem nyit XSS-lehetőséget."""
    return (_esc(text)
            .replace("\r\n", "\n")
            .replace("\r", "\n")
            .replace("\n", "<br>"))
...
def tetel_sor(item: dict) -> str:
    ...
    if megjegyzes:
        nev_html = (
            ...
            f'<br><span style="font-size:9.5px;color:#8a8a8a;">{_esc_multiline(megjegyzes)}</span></div>'
        )
    else:
    ...
    if leiras:
        html += (
            ...
            f'padding:7px 8px 13px;">{_esc_multiline(leiras)}</div>'
        )
    else:
...
def szekcio_sor(item: dict) -> str:
    """
    Egy SZEKCIÓCÍM sor HTML-je - pl. "I. A magasraktár keskenyfolyosóinak
    síkpontossági kialakítása:", "Opcionális tétel:", "Megjegyzések és egyéb
    feltételek".

    2026.08.14-i bevezetés: az Innonest árajánlaton ezek teljes szélességű,
    kiemelt elválasztó sorok a tételek között. A generátor eddig egyáltalán
    nem vitte át őket (a szelektor csak a "tr.items" sorokat nézte) - lásd a
    felismerésük részleteit a _scrape_arajanlat kommentjében.

    A "break-after:avoid" azért kell, hogy a szekciócím ne szakadjon el
    oldaltöréssel az alatta következő első tételtől.
    """
    cim = _esc(item.get("megnevezes"))
    if not cim:
        return ""
    return (
        '<div style="break-inside:avoid;break-after:avoid;'
        'margin:14px 0 0;padding:8px 8px 7px;'
        'border-top:2px solid #2e2c2c;border-bottom:1px solid #d9d9d9;'
        'font-size:11.5px;font-weight:700;color:#2e2c2c;letter-spacing:.2px;">'
        f'{cim}</div>'
    )
...
def build_arajanlat_html(adatok: dict) -> str:
    ...
    # 2026.08.14: a renderelés a "sorok" listából megy, ami a tételeket ÉS a
    # szekciócímeket is tartalmazza, az Innonestben látott sorrendben.
    # Ha a "sorok" hiányzik (régebbi adatszerkezet / régi job), visszaesünk a
    # csak-tételes listára, hogy semmiképp ne törjön el a generálás.
    sorok = adatok.get("sorok") or [dict(it, tipus="tetel") for it in items]
    tetelek_html = "".join(
        szekcio_sor(s) if s.get("tipus") == "szekcio" else tetel_sor(s)
        for s in sorok
    )
...
async def _find_bid_page(page, bid: str, max_pages: int = 20) -> bool:
    """Végiglapoz az Innonest bids listán, és megáll azon az oldalon ahol a BID megtalálható.
    Visszatér True-val ha megtalálta, False-szal ha nem."""
    for page_num in range(max_pages):
        offset = page_num * 100
        url = "https://app.innonest.hu/bids" if offset == 0 else f"https://app.innonest.hu/bids/index/{offset}/"
        ...
        found = await page.evaluate(
            ...
            (bid) => {
                ...
                for (const td of allTds) {
                    if (bidRegex.test(td.innerText.trim())) return true;
                }
                return false;
            }
            """,
        ...
        if found:
            log.info(f"[PDF] BID {bid} megtalálva: oldal {page_num + 1} (offset {offset})")
            return True

        # Ha nincs több oldal (nincs "Következő" link vagy üres a lista), megállunk
        has_next = await page.evaluate(
            "() => !!document.querySelector('a[href*=\"/bids/index/\"]')"
        )
        if not has_next:
            break
...
async def _find_bid_edit_url(page, bid: str) -> str:
    """A 'bids' listán megkeresi a BID sort (lapozással), és visszaadja a szerkesztő URL-jét."""
    found = await _find_bid_page(page, bid)
    if not found:
    ...
    if not bid_link:
        raise Exception(f"BID sor megtalálva, de nincs szerkesztő link: {bid}")
...
async def _scrape_arajanlat(page, bid: str) -> dict:
    ...
    # ── Tételek + SZEKCIÓCÍMEK (dokumentum-sorrendben) ──
    # 2026.08.14-i JAVÍTÁS: a korábbi szelektor csak a tétel-sorokat vette
    # ('tbody.items-box tr.items:not([data-id="0"])'), ezért az Innonest
    # SZEKCIÓCÍM-sorai (pl. "I. A magasraktár keskenyfolyosóinak síkpontossági
    # kialakítása:", "Opcionális tétel:", "Megjegyzések és egyéb feltételek")
    # teljesen kimaradtak a generált PDF-ből, holott az Innonest saját PDF-jén
    # ott vannak (a felhasználó ezt jelezte).
    #
    # Élő DOM-vizsgálattal tisztázva (BID-2026-272 =
    # app.innonest.hu/bids/change/11261000, 2026.08.14):
    #   - SZEKCIÓCÍM sor : NINCS rajta "items" osztály, NINCS productsQty input,
    #                      csak productsName_N + elementsTypeId_N (értéke: 3)
    #   - normál TÉTEL   : van "items" osztály ÉS van productsQty input
    #   - data-id="0"    : rejtett sablonsor, ezt továbbra is kihagyjuk
    # Ezért most VÉGIGMEGYÜNK az összes soron, és típussal jelöljük őket -
    # így a szekciócímek és a tételek eredeti SORRENDJE is megmarad.
    sorok = await page.evaluate(
        r"""
        () => {
            const trs = document.querySelectorAll('tbody.items-box tr');
            const out = [];
            trs.forEach(tr => {
                if (tr.getAttribute('data-id') === '0') return;   // rejtett sablonsor
                ...
                // Szekciócím: nincs mennyiség-mezője és nincs "items" osztálya
                if (!mennyInput && !tr.classList.contains('items')) {
                    out.push({ tipus: 'szekcio', megnevezes: nev });
                    return;
                }
                ...
                // FONTOS: a leírás sortöréseit MEGTARTJUK (csak a két végét
                // vágjuk le) - a HTML-be a _esc_multiline() teszi majd <br>-ként.
    ...
    tetelek  = [s for s in sorok if s.get("tipus") != "szekcio"]
    szekciok = [s for s in sorok if s.get("tipus") == "szekcio"]

    eredmeny["sorok"]   = sorok
    eredmeny["tetelek"] = tetelek
    log.info(f"[PDF] {len(tetelek)} tétel és {len(szekciok)} szekciócím kiolvasva")
    for sz in szekciok:
        log.info(f"[PDF]   [szekció] {sz['megnevezes'][:60]!r}")
...
async def _upload_pdf_to_innonest(page, pdf_path: str, bid: str, ugyfel_nev: str = ""):
    ...
    if bid_index < 0:
        raise Exception(f"PDF feltöltés: BID sor megtalálva, de trigger link hiányzik: {bid}")
