...
<script>
  ...
  if (p.get('err')) {
    ...
    @app.route("/pipedrive-deal-webhook", methods=["POST"])
    def pipedrive_deal_webhook():
        ...
        # 2) Token generálás és adatok memóriában tárolása
        ...
        # 3) Import URL összeállítása és visszaírás Pipedrive-ba
        base       = (WEBAPP_BASE_URL or "").rstrip("/")
        import_url = f"{base}/?pd_import={token}"

        try:
            _pd_write_webapp_url(deal_id, import_url)
        except Exception as e:
            log.warning(f"[PD] URL visszaírás sikertelen (import token mégis él): {e}")

        return jsonify({"ok": True, "token": token, "import_url": import_url})
