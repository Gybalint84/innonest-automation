...
def _sig_tokens(s: str) -> set:
    """Jelentős (≥5 betűs) szavak halmaza a megnevezés/számlaszöveg egyezéséhez."""
    toks = re.findall(r"[a-záéíóöőúüű0-9]{5,}", (s or "").lower())
    return {t for t in toks if t not in _DESC_STOP}
...
def _match_sheet_bid(invoice_text: str, netto: float, currency: str,
                     partner: str, datum: str, sheets: list) -> str:
    """Billingo számla → Innonest megrendelőlap párosítás, több jelzés alapján,
    a legmegbízhatóbbtól az esélyesebb becslésig:
      1) PONTOS/KÖZELI ÖSSZEG + deviza (teljes számla). Több egyezésnél partner+dátum dönt.
      2) PARTNER + MEGNEVEZÉS egyezés — a számla szövegében szerepel-e a megrendelőlap
         jellemző szava (pl. „betoncsiszolás"). Így a részszámla is megtalálja a BID-et,
         ha a számlán rajta van a munka megnevezése.
      3) PARTNER + egyetlen megrendelőlap — ha a vevőnek csak egy nyitott munkája van.
      4) PARTNER + legközelebbi (a számlánál nem kisebb) összeg — részszámla-becslés.
    """
    if not sheets:
        return ""
    ...
    partner_sheets = [s for s in sheets if _same_partner(partner, s.get("ugyfel", ""))]

    # 1) Pontos/közeli összeg + deviza.
    if netto:
        amt = [s for s in sheets if s["currency"] == currency
               and abs(s["netto"] - netto) <= max(5.0, netto * 0.001)]
        if len(amt) == 1:
            return amt[0]["bid"]
        if len(amt) > 1:
            by = [s for s in amt if _same_partner(partner, s.get("ugyfel", ""))] or amt
            if len(by) == 1:
                return by[0]["bid"]
            it = _day(datum)
            by.sort(key=lambda s: abs(_day(s["datum"]) - it) if it and s["datum"] else 1e18)
            return by[0]["bid"]

    # 2) Partner + megnevezés (leírás) token-egyezés.
    inv_tok = _sig_tokens(invoice_text)
    if partner_sheets and inv_tok:
        scored = sorted(
            ((len(inv_tok & _sig_tokens(s.get("desc", ""))), s) for s in partner_sheets),
            key=lambda x: x[0], reverse=True,
        )
        if scored and scored[0][0] >= 1 and (len(scored) == 1 or scored[0][0] > scored[1][0]):
            return scored[0][1]["bid"]

    # 3) Partner + egyetlen megrendelőlap.
    if len(partner_sheets) == 1:
        return partner_sheets[0]["bid"]

    # 4) Partner + legközelebbi (a számlánál nem kisebb) összeg — részszámla-logika.
    if netto and partner_sheets:
        near = [s for s in partner_sheets if s["currency"] == currency and s["netto"] >= netto - 5]
        if near:
            near.sort(key=lambda s: s["netto"] - netto)
            return near[0]["bid"]
    return ""
...
def _collect_month(month: str) -> list:
    ...
    while True:
        ...
        for d in docs:
            ...
            netto      = _doc_net(full)
            # 1) szöveges BID a számlán (ha rákerült), 2) többjelzéses megrendelőlap-párosítás
            bid = _extract_bid(full) or _match_sheet_bid(
                _invoice_text(full), netto, _doc_currency(full), partner, datum, sheets
            )
            items.append({
