...
async def run_automation(payload: dict):
    ...
    # 2026-09-04: azon STO/Eurostep/Murexin anyagok nevei, amiket az
    # autocomplete-keresés (_select_material_from_autocomplete) NEM talált
    # meg az Innonest raktárkészletében — ezeket a válaszban visszaadjuk,
    # hogy a webapp ki tudja írni a felhasználónak (lásd a hívó
    # Arajanlat.jsx: InnonestButton komponenst).
    unmatched_materials = []
    ...
    async with async_playwright() as p:
        ...
        if items[0].get("hideFromCustomer"):
            ...
            if row0 is not None:
                matched0 = await _select_material_from_autocomplete(page, row0, items[0]["megnevezes"])
                if not matched0:
        ...
        for i, item in enumerate(items[1:], start=1):
            ...
            if item.get("hideFromCustomer"):
                ...
                if row_n is not None:
                    matched_n = await _select_material_from_autocomplete(page, row_n, item["megnevezes"])
                    if not matched_n:
    ...
    if unmatched_materials:
        log.warning(
            f"[ANYAG-VALASZTAS] Összesen {len(unmatched_materials)} anyag nem volt "
            f"rögzítve termékként az Innonestben: {unmatched_materials}"
        )
