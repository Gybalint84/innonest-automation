...
async def _write_megjegyzes(page, bid_szam: str, szoveg: str, bid_url: str = ""):
    ...
    # Mehet! gomb keresése — MIELŐTT a textarea-t kitöltjük, hogy biztos látható legyen
    ...
    # Textarea kitöltése — press_sequentially valódi billentyűleütéseket szimulál
    # (fill() esetenként nem triggereli a JS input event-et)
    await textarea.click()
    await textarea.select_all()
    await page.wait_for_timeout(100)
    await textarea.press_sequentially(szoveg, delay=20)
    ...
    if mehet_cnt > 0:
        ...
        # Ellenőrzés: a panel bezárult-e? (ha igen, valószínűleg mentett)
        still_open = await page.locator('textarea[placeholder="Megjegyzés"]:visible').count()
        log.info(f"[MEGJEGYZES] Panel bezárult mentés után: {still_open == 0}")
