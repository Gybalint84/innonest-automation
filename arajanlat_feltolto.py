...
async def _select_material_from_autocomplete(page, row, megnevezes: str) -> bool:
    ...
    try:
        ...
        await name_input.type(megnevezes, delay=60)

        # Várakozás a legördülő találatokra (keresés-debounce + XHR válasz) —
        # legfeljebb kb. 2,4 mp-et pollozunk, hogy ne várjunk feleslegesen sokat.
        candidate_count = 0
        for _ in range(8):
        ...
        # ⚠️ 2026-09-04, javítás: az első verzió a "Termék mennyiség" feliratot
        # tartalmazó LEGSZŰKEBB elemet vette "kártyának" — de ez valójában csak
        # a statisztika-sor (a terméknév egy TESTVÉR elemben van), ezért soha
        # nem volt benne a keresett név, és minden találat "nem egyezik"-nek
        # tűnt (élesben minden anyagnál "1 jelölt látszott, de nincs egyezés").
        # Most a stat-sortól FELFELÉ lépkedünk a szülőláncon, amíg egy nagyobb
        # (a nevet is tartalmazó) konténert nem találunk. Ha ez sem talál
        # egyezést, diagnosztikai adatokat is visszaadunk (a stat-elem és a
        # szülei szövege/class-a), hogy a Railway logból pontosítható legyen.
        pick_result = await page.evaluate(
            ...
            (nev) => {
                ...
                // 1) Levél-szintű "stat" elemek, amik szó szerint tartalmazzák
                //    a "Termék mennyiség" feliratot.
        ...
        if not pick_result.get("picked"):
            log.warning(
                f"[ANYAG-VALASZTAS] Nem találtam egyező legördülő találatot: '{megnevezes}' "
                f"({pick_result.get('candidates', candidate_count)} kártya, "
                f"{pick_result.get('statCount')} stat-elem) — a mező a beírt szöveggel maradt, "
                f"ELLENŐRZÉS SZÜKSÉGES (lehet, hogy új anyagként jön létre)."
            )
            diag = pick_result.get("diag") or []
            for d in diag:
                log.warning(
        ...
        log.info(
            ...
            f"egyezés): '{megnevezes}' -> {pick_result.get('text')}"
        )
        return True
...
async def _reapply_row_values(page, row, item: dict) -> None:
    ...
    try:
        ...
        log.info(f"[ANYAG-VALASZTAS] Mennyiség/ár visszaírva a kiválasztás után: '{item['megnevezes']}'")
    except Exception as e:
        log.warning(f"[ANYAG-VALASZTAS] Hiba az érték-visszaírásnál ('{item.get('megnevezes')}'): {e}")
