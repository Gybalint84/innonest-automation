...
# ══════════════════════════════════════════════════════════════════════════════
# TÉTEL ELREJTÉSE AZ ÜGYFÉL ELŐL (STO/Eurostep/Murexin beszerzési anyagsorok)
# ══════════════════════════════════════════════════════════════════════════════
#
...
# `hideFromCustomer: true` jön, és ilyenkor a sor felvitele UTÁN rákattintunk
# az Innonest „ügyfél elől elrejt" (áthúzott szem, kék) ikonjára a soron.
#
...
async def _toggle_hide_from_customer(page, megnevezes: str) -> bool:
    try:
        click_result = await page.evaluate(
            ...
            (nev) => {
                ...
                // Ha ikon-belső elemre (i/svg/path) találtunk, a kattintható
                // szülőt (button/a) keressük.
                let btn = hit;
...
async def run_automation(payload: dict):
    ...
    async with async_playwright() as p:
        ...
        if items[0].get("hideFromCustomer"):
            await _toggle_hide_from_customer(page, items[0]["megnevezes"])
        ...
        for i, item in enumerate(items[1:], start=1):
            ...
            # A STO/Eurostep/Murexin beszerzési-áras anyagsorokat elrejtjük az
            # ügyfél elől (lásd _toggle_hide_from_customer fejléc-kommentje).
            if item.get("hideFromCustomer"):
                await _toggle_hide_from_customer(page, item["megnevezes"])
