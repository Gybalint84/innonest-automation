...
async def run_automation(payload: dict):
    ...
    async with async_playwright() as p:
        ...
        # Tételek
        # ── Template sorok törlése ──────────────────────────────────────────────
        # Az Innonest "Új árajánlat" formja 1-2 üres template sort tölt be (data-id="0").
        # Ha ezeket üresen hagyjuk, az Innonest elutasítja a mentést.
        # Megoldás: töröljük a template sorokat mielőtt hozzáadjuk a saját tételeinket.
        template_torolve = await page.evaluate("""
            () => {
                // data-id="0" = Innonest template sor
                const templateSorok = document.querySelectorAll(
                    'tbody.items-box tr.items[data-id="0"], ' +
                    'tr.items[data-id="0"]'
                );
                let db = 0;
                templateSorok.forEach(function(row) {
                    // Próbáljuk meg a delete gombot kattintani
                    var delBtn = row.querySelector(
                        'button[class*="delete"], button[title*="törl"], ' +
                        'button[title*="Delete"], .fa-trash, button.btn-danger'
                    );
                    if (delBtn) {
                        delBtn.click();
                    } else {
                        // Ha nincs delete gomb, eltávolítjuk a DOM-ból
                        row.remove();
                    }
                    db++;
                });
        ...
        """)
        if template_torolve > 0:
            log.info(f"Template sorok törölve: {template_torolve} db")
            await page.wait_for_timeout(400)

        # Ha nem volt data-id="0" alapján azonosítható sor, próbáljuk meg
        # az üres megnevezés mezők alapján (fallback)
        if template_torolve == 0:
            template_torolve2 = await page.evaluate("""
                () => {
                    const nevInputok = document.querySelectorAll(
                        'input[placeholder="Tétel megnevezése"]'
                    );
                    let db = 0;
                    nevInputok.forEach(function(inp) {
                        if (!inp.value || !inp.value.trim()) {
                            var sor = inp.closest('tr');
                            if (sor) { sor.remove(); db++; }
                        }
                    });
                    return db;
                }
            """)
            if template_torolve2 > 0:
                log.info(f"Üres sorok törölve (fallback): {template_torolve2} db")
                await page.wait_for_timeout(400)
