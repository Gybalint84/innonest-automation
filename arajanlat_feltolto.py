...
async def _write_megjegyzes(page, bid_szam: str, szoveg: str):
    ...
    # FONTOS: csak button/a tagre illeszkedünk, hogy ne kattintsunk wrapper div-re!
    comment_btn = None
    for sel in [
        'button[title*="egjegyz"]', 'a[title*="egjegyz"]',
        'button[title*="omment"]', 'a[title*="omment"]',
        'button[class*="comment"]', 'a[class*="comment"]',
        'button[class*="speech"]', 'a[class*="speech"]',
        'button[class*="bubble"]', 'a[class*="bubble"]',
    ]:
    ...
    if comment_btn is None:
        # Fallback: innerHTML alapján azonosítjuk a gombokat (pl. fa-comment ikon)
        btns = row.locator('button')
        cnt = await btns.count()
        log.info(f"[MEGJEGYZES] {cnt} gomb a sorban, innerHTML vizsgálat...")
        for idx in range(cnt):
            btn = btns.nth(idx)
            try:
                inner = await btn.inner_html()
                log.info(f"[MEGJEGYZES] Gomb {idx} HTML: {inner[:120]}")
                if any(kw in inner.lower() for kw in ['comment', 'speech', 'bubble', 'megjegyz', 'fa-comment']):
                    comment_btn = btn
                    log.info(f"[MEGJEGYZES] Ikon innerHTML alapján: gomb {idx}")
                    break
            except Exception:
                pass

        # Ha még mindig nem találtuk, 2. gomb (csatolmány=1., megjegyzés=2.)
        if comment_btn is None and cnt >= 2:
            comment_btn = btns.nth(1)
            log.info(f"[MEGJEGYZES] Fallback: 2. gomb a sorban")
