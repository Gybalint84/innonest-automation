    ...
    try:
        ...
        pick_result = await page.evaluate(
            ...
            (nev) => {
                ...
                // "Kártya" elemek: láthatóak, és tartalmazzák a jellegzetes
                // "Termék mennyiség" feliratot — a legszűkebb ilyen elemeket
                // vesszük (ne az egész legördülő listát válasszuk ki).
                ...
                if (!hit) return { picked: false, candidates: cards.length };
                hit.click();
        ...
        if not pick_result.get("picked"):
            log.warning(
                ...
                f"({pick_result.get('candidates', candidate_count)} jelölt látszott) — a mező a "
                f"beírt szöveggel maradt, ELLENŐRZÉS SZÜKSÉGES (lehet, hogy új anyagként jön létre)."
            )
            return False
