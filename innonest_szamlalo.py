...
def _billingo_invoiced_bids(megrendelesek: list, current_year: int) -> set:
    """Visszaadja azoknak a BID-eknek a halmazát, amelyekre a tárgyévben
    Billingóban számla készült.
                ...
                for d in docs:
                    ...
                    if bid:
                        ...
                        log.info(
                            ...
                            f"{netto} {_doc_currency(full)} | {datum}"
                        )
    ...
    return invoiced_bids


def _apply_billingo_invoiced(data: dict) -> None:
    """A Billingóban kiszámlázott megrendelések Státuszát "Számlázva"-ra állítja
    (helyben módosítja a data dict "megrendelesek" listáját).
    ...
    invoiced_bids = _billingo_invoiced_bids(megrendelesek, current_year)
