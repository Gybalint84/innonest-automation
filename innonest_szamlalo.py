...
1) GET /innonest-counters
   A két listaoldal LEGFELSŐ sorából (alapból legújabb elöl rendezve) kiolvassa
   az idei ajánlatok és megrendelések darabszámát:
...
2) GET /innonest-full-data
   A két listaoldal ÖSSZES (az oldalon látható, idei) sorát kiolvassa és
   párosítja: minden megrendelőlaphoz a "Megnevezés" mező végén automatikusan
   megjelenő "[... #BID-2026-155]" jelölésből kinyeri, melyik ajánlathoz
   tartozik. Nincs lapozás (a felhasználó megerősítette: az első oldal mindig
   a legfrissebbeket mutatja, ez elegendő - "nem szükséges [lapozni] egyenlőre,
   mert az első oldalon mutatja mindig a legfrissebbet").
...
async def _scrape_rows(page, url: str):
    """Egy listaoldal (bids vagy ordersheets) összes sorát kiolvassa: az
    azonosítót ("td.left.bold a" szövege) és a teljes sor szövegét (innerText),
    a megrendeles_figyelő.py-ban bevált teljes-sor-regex mintát követve."""
    await page.goto(url, wait_until="networkidle")
...
async def _scrape_ajanlatok(page, current_year: int):
    raw_rows = await _scrape_rows(page, ARAJANLATOK_URL)
    out = []
    for r in raw_rows:
        bid = r["id"]
...
async def _scrape_megrendelesek(page, current_year: int):
    raw_rows = await _scrape_rows(page, MEGRENDELOLAPOK_URL)
    out = []
    for r in raw_rows:
        rendelesszam = r["id"]
