...
# 2026.08.27: a cég visszaállt Számlázz.hu-ra, így az Innonest /invoices lista
# ("Számlák (Számlázz.hu)") ismét TARTALMAZZA az összes idei számlát (élőben
# ellenőrizve: 90 db idei számla, ~104,6 M Ft HUF + ~160 900 EUR nettó, a
# sztornókat előjelhelyesen kezelve → összesen ~167 M Ft). Ezért a
# számlázottságot ÉS a TÉNYLEGES kiszámlázott nettót ismét innen olvassuk
# (lásd _scrape_szamlak / _apply_innonest_invoiced), NEM a Billingóból.
INNONEST_INVOICES_ENABLED = True
...
# 2026.08.27: számla-soronkénti nettó parse a Számlázz.hu (/invoices) listához.
# Két ponton tér el a megrendelőlap-parse-tól:
#  1) ELŐJELHELYES: a sztornó számlák nettója NEGATÍV (pl. "-4 528 428 HUF"),
#     így az eredeti + sztornó pár az összegben kioltja egymást (a korrekció marad).
#  2) A nettó a sorban szereplő értékek közül a KISEBB ABSZOLÚT ÉRTÉKŰ (előjelet
#     megtartva) - a sima min() negatív összegeknél a bruttót adná vissza.
# Az ezres elválasztó lehet sima szóköz vagy NBSP is (a felület mindkettőt használja).
_INV_HUF_RE = re.compile(r"(-?\d[\d  ]*\d)\s*HUF")
_INV_EUR_RE = re.compile(r"(-?\d(?:[\d  ]*\d)?(?:[.,]\d{1,2})?)\s*EUR")


def _parse_invoice_netto(text: str):
    """(nettó, deviza) egy Számlázz.hu számla-sor teljes szövegéből, ELŐJELHELYESEN.
    HUF-ban a sorban szereplő értékek (nettó/bruttó) közül a kisebb abszolút értékű
    a nettó; sztornónál ez negatív. Ha nincs HUF összeg, EUR-t próbál. Ha semmi
    értelmezhető összeg nincs: (0.0, "")."""
    def _nums(rx):
        out = []
        for m in rx.finditer(text):
            ...
            try:
                ...
                continue
        return out
    huf = _nums(_INV_HUF_RE)
    if huf:
        return min(huf, key=abs), "HUF"
    eur = _nums(_INV_EUR_RE)
    if eur:
        return min(eur, key=abs), "EUR"
    return 0.0, ""
...
async def _scrape_szamlak(page, current_year: int, max_pages: int = 50):
    """2026.08.27: végiglapozza a Számlázz.hu számlalistát (/invoices) és
    kigyűjti a tárgyévi számlákat a TÉNYLEGES kiszámlázott nettó összegzéséhez.
    Soronként eltárolja: a kiállítás dátumát (Kelte = az első dátum a sorban), a
    nettót az EREDETI devizában (előjelhelyesen, sztornó = negatív), hogy sztornó-e,
    és a sorban szereplő BID-/megrendelés-hivatkozásokat (a projekt 'Számlázva'
    állapotához). Az EUR→HUF átváltást NEM itt, hanem a böngésző bezárása UTÁN
    (_apply_innonest_invoiced) végezzük, hogy a blokkoló árfolyam-HTTP ne a
    Playwright event loopot terhelje. A lista legújabb elöl rendezett, ezért az
    első korábbi évi sornál megállunk."""
    szamlak = []
    offset = 0
    for _ in range(max_pages):
        raw_rows = await _scrape_rows(page, _invoices_page_url(offset))
        if not raw_rows:
            break
        ...
        for r in raw_rows:
            ...
            if not date_m:
                continue
            netto_orig, currency = _parse_invoice_netto(text)
            if not currency:
                continue
            bids, orders = _extract_invoice_refs(text)
            szamlak.append({
            ...
            while True:
                ...
                for d in docs:
                    ...
                    if currency == "HUF":
                        ...
                        # Devizás számla: ELSŐSORBAN a Billingo SAJÁT árfolyamát
                        # használjuk (conversion_rate) - így a végösszeg pontosan
                        # azt adja, amit a Billingo felületén látsz. Csak ha ez
                        # hiányzik, esünk vissza az ECB/Frankfurter árfolyamra
                        # (_eur_huf_rate_for_date), ami közelítés.
                        # 2026.08.14-i finomítás: az első körben csak az ECB-
                        # árfolyammal számoltunk, és emiatt a végösszeg 1 007 675
                        # Ft-tal (0,6%) eltért a Billingo által mutatott értéktől
                        # (155 651 440 vs 156 659 115) - az eltérés kizárólag a
                        # devizás számlák átváltásából jött.
                        rate = full.get("conversion_rate")
                        rate = float(rate) if isinstance(rate, (int, float)) and rate else None
                        rate_forras = "Billingo conversion_rate"
                        if not rate and currency == "EUR":
                            rate = _eur_huf_rate_for_date(datum)
                            rate_forras = "ECB/Frankfurter (közelítés)"
                        if rate:
                            netto_ft_osszesen += round(netto * rate)
                            log.info(
                                f"[SZAMLALO] Devizás számla átváltva: "
                                f"{full.get('invoice_number')!r} | {netto} {currency} "
                                f"x {rate} = {round(netto * rate):,} Ft [{rate_forras}]".replace(",", " ")
                            )
                        else:
                            log.error(
...
def _apply_innonest_invoiced(data: dict) -> None:
    """2026.08.27 – a Billingó HELYETT a Számlázz.hu (/invoices) listából számol
    (a cég visszaállt Számlázz.hu-ra). A data["_szamlak"] (lásd _scrape_szamlak)
    alapján, helyben módosítva a data dict-et:

      1) Összegzi a TÉNYLEGES kiszámlázott nettót Ft-ban – idei, halmozottan
         (data["szamlazott_netto_ft"]) ÉS külön az AKTUÁLIS naptári hónapra
         (data["szamlazott_netto_ft_akt_ho"]) –, plusz a számlák darabszáma
         (data["szamlazott_szamla_db"]). EUR számláknál a számla keltének napi
         árfolyamával vált (_eur_huf_rate_for_date), ELŐSZÖR itt, a böngésző
         bezárása UTÁN, hogy a blokkoló HTTP ne a Playwright event loopot terhelje.
         Sztornó = negatív, tehát az eredeti + sztornó pár kioltja egymást.
      2) A számlázott BID-/megrendelésszám-hivatkozások alapján a megfelelő
         megrendelések Státuszát "Számlázva"-ra állítja (a "Futó projektek" KPI-hoz).

    Hibatűrés: ha nincs _szamlak (a lekérés ki volt kapcsolva vagy elbukott), NEM
    állítunk be pénzügyi mezőt – a dashboard "Leszámlázott" kártyája ilyenkor "–"-t
    mutat, nem 0-t vagy régi értéket."""
    szamlak = data.pop("_szamlak", None)
    if szamlak is None:
        log.warning("[SZAMLALO] Nincs Számlázz.hu számlalista (_szamlak) - a "
                    "Leszámlázott kártya adat nélkül marad ('–').")
        return
    ...
    for sz in szamlak:
        datum = sz.get("datum") or ""
        currency = sz.get("currency") or ""
        val = float(sz.get("netto_orig") or 0.0)
        ...
        if currency == "HUF":
            ...
            rate = _eur_huf_rate_for_date(datum)
            if not rate:
                log.error(f"[SZAMLALO] Számla EUR->HUF árfolyam hiányzik ({datum}), "
                          f"{val} EUR kimarad a leszámlázott összegből - kézi ellenőrzés kell.")
                ft = 0
            else:
                ft = round(val * rate)
        else:
        ...
        netto_ft += ft
        try:
            if int(datum[:4]) == current_year and int(datum[5:7]) == current_month:
                netto_ft_akt_ho += ft
        except (ValueError, IndexError):
            pass

        if not sz.get("is_storno"):
            szamla_db += 1
            if val > 0:  # csak valós (nem kioltott) számla jelöl projektet számlázottnak
                invoiced_bids |= set(sz.get("bids") or [])
                invoiced_orders |= set(sz.get("orders") or [])

    data["szamlazott_netto_ft"] = netto_ft
    data["szamlazott_netto_ft_akt_ho"] = netto_ft_akt_ho
    data["szamlazott_szamla_db"] = szamla_db

    ujonnan = 0
    for m in (data.get("megrendelesek") or []):
        mar = (m.get("allapot") == "Számlázva")
        by_inv = (bool(m.get("bid")) and m["bid"] in invoiced_bids) or (m.get("rendelesszam") in invoiced_orders)
        m["szamlazva"] = mar or by_inv
        if by_inv and not mar:
            m["allapot"] = "Számlázva"
            ujonnan += 1

    log.info(
        f"[SZAMLALO] Számlázz.hu összegzés: {szamla_db} számla, idei nettó "
        f"{netto_ft:,} Ft, aktuális havi {netto_ft_akt_ho:,} Ft, "
        f"{ujonnan} megrendelés újonnan 'Számlázva'.".replace(",", " ")
    )
...
async def _scrape_megrendelesek(page, current_year: int):
    ...
    for r in raw_rows:
        ...
        # 2026.08.27: ha a megrendelőlap sorában nincs felismerhető státusz-szó,
        # NE maradjon üres a Projektek lap Státusza (a dashboardon ez okozta a
        # sok üres sort) - egy megrendelőlap alapból legalább "Megrendelt". A
        # ténylegesen kiszámlázottakat ezután a _apply_innonest_invoiced írja át
        # "Számlázva"-ra a Számlázz.hu számlák alapján.
        allapot = _parse_status(text) or "Megrendelt"
        lines = _meaningful_lines(text)
        targy = lines[0] if len(lines) >= 1 else ""
        ugyfel = lines[1] if len(lines) >= 2 else ""
        out.append({
...
async def _innonest_full_data_async() -> dict:
    async with async_playwright() as pw:
        ...
        # 2026.08.27: a cég visszaállt Számlázz.hu-ra, így az Innonest /invoices
        # lista ("Számlák (Számlázz.hu)") ismét TARTALMAZZA az összes idei számlát.
        # Innen olvassuk ki a TÉNYLEGES kiszámlázott nettót (rész-/gyűjtő- és
        # sztornó-számlákkal együtt, előjelhelyesen), NEM a Billingóból. A nyers
        # számlalistát átadjuk; az összegzést + EUR-átváltást + a 'Számlázva'
        # jelölést a szinkron burkoló (_apply_innonest_invoiced) végzi, MIUTÁN a
        # böngésző bezárult - így a blokkoló árfolyam-HTTP nem a Playwright event
        # loopot terheli.
        szamlak = await _scrape_szamlak(page, current_year) if INNONEST_INVOICES_ENABLED else None
...
def innonest_full_data() -> dict:
    ...
    # 2026.08.27: a számlázottság jelölése + a TÉNYLEGES kiszámlázott nettó a
    # Számlázz.hu (/invoices) listából - SZÁNDÉKOSAN itt, a run_in_loop UTÁN, hogy
    # a blokkoló EUR/HUF árfolyam-HTTP ne a Playwright-tel közös event loopon
    # fusson. Saját try/except: ha ez a lépés elbukik, a már sikeresen lekapart
    # ajánlat-/megrendelés-lista akkor is visszamegy a Sheets sync scriptnek (a
    # Leszámlázott kártya ilyenkor '–'). A régi _apply_billingo_invoiced dormant
    # marad a fájlban, ha a cég valaha visszaállna Billingóra.
    try:
        _apply_innonest_invoiced(data)
    except Exception as e:  # noqa: BLE001
        log.error(f"[SZAMLALO] Számlázz.hu számla-összegzés hiba: {e} - a megrendelések "
                  f"a saját Innonest-státuszukkal mennek tovább, a Leszámlázott kártya '–'.")
        data.pop("_szamlak", None)
