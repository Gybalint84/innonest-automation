...
megbízható jelzés arra, hogy egy projekt valóban lezárult-e. Mostantól egy
külön lépésben végignézzük az Innonest Számlák listáját is
(https://app.innonest.hu/invoices/index/0/, .../index/100/, ... - lásd
_invoices_page_url, ugyanaz a lapozási/táblaszerkezet, mint a bids/ordersheets
listáknál), és a benne szereplő BID-szám és/vagy megrendelésszám-hivatkozások
alapján (lásd _extract_invoice_refs, _scrape_invoiced_refs) állapítjuk meg,
mely megrendelések vannak TÉNYLEGESEN kiszámlázva. Ha egy megrendelés
számlázottnak bizonyul, a Státuszát felülírjuk "Számlázva"-ra (lásd
_innonest_full_data_async) - ez a Power BI dashboard CLOSED_PROJECT_STATUSES
listájával (Teljesítve/Számlázva/Törölve/Visszautasítva) találkozva
automatikusan kikerül a "Futó projektek" számlálásból, külön dashboard-oldali
módosítás nélkül. FONTOS, MÉG ÉLŐBEN NEM ELLENŐRZÖTT RÉSZ: a Számlák lista
sorainak pontos szövegformátumát (hogyan hivatkozik a BID-re/rendelésszámra)
feltételezés alapján dolgoztuk fel - ha élesben nem talál egyezést, elsőként
egy valós Innonest-képernyőkép (Számlák lista, egy konkrét sor teljes
szövegével) alapján kell pontosítani az INVOICE_REF_PATTERN mintát, a korábbi
EUR-minta hibajavításához hasonlóan.
"""
...
# Ez a minta (a BID_REF_PATTERN-től eltérően) NEM várja el a záró "]"-t, mert
# a számla-oldal sorformátuma még nincs élőben leellenőrizve - ha a valós
# szerkezet eltérne, ezt kell majd pontosítani egy Innonest-képernyőkép alapján
# (lásd a modul docstringjében a korábbi EUR-minta hasonló tanulságát).
...
async def _scrape_invoiced_refs(page, current_year: int, max_pages: int = 50):
    ...
    nézni: ha egy megrendeléshez (BID vagy rendelésszám alapján) VAN számla a
    Számlák listában, azt tekintjük lezártnak ("Számlázva"), függetlenül attól,
    mit mutat a megrendelőlap saját állapot-szövege - lásd a hívó
    _innonest_full_data_async-ban a felülírást.

    A lista (a többihez hasonlóan) legújabb elöl rendezett - amint egy
    korábbi évre eső dátumú sort találunk, megállunk (a további oldalakon már
    csak régebbi évek jönnének). Nyitott kérdés: ha egy idei megrendelést csak
    JÖVŐRE számláznak ki, azt csak a következő évi futás fogja látni - ezzel
...
async def _innonest_full_data_async() -> dict:
    async with async_playwright() as pw:
        ...
        # 2026.07.07-i módosítás: a "Számlázva" állapotot mostantól a tényleges
        # Számlák listával (/invoices) igazoljuk vissza, nem a megrendelőlap
        # saját Státusz-szövegére hagyatkozunk - lásd _scrape_invoiced_refs
        # docstringjét. Ha egy megrendelés BID-je vagy rendelésszáma szerepel a
        # Számlák valamelyik sorában, a Státuszt "Számlázva"-ra állítjuk,
        # függetlenül attól, mit mutatott addig a megrendelőlap szövege
        # (de fordítva NEM töröljük a "Számlázva"-t, ha a megrendelőlap már
        # eleve azt mutatta, csak az /invoices lista nem erősítette meg -
        # elővigyázatosságból, amíg ez a logika élesben nincs leellenőrizve).
