...
async def get_arajanlat_tetelek(page, bid: str) -> dict:
    ...
    try:
        ...
        tetelek = await page.evaluate("""
            () => {
                ...
                sorok.forEach(tr => {
                    ...
                    // Egység kinyerése – több lehetséges helyről
                    let egys = '';
                    const egysInput = tr.querySelector(
                        'input[name^="productsUnit"], select[name^="productsUnit"], ' +
                        'input[name^="unit"], select[name^="unit"]'
                    );
                    if (egysInput) {
                        egys = (egysInput.value || egysInput.selectedOptions?.[0]?.text || '').trim();
                    }
                    if (!egys) {
                    ...
                    if (!egys) {
                        const anySmall = tr.querySelector('div.smaller');
                        if (anySmall) {
                            const t = anySmall.innerText.trim();
                            const m = t.match(/[A-Za-z]{1,5}\\d*$/i);
                            egys = m ? m[0].toLowerCase() : '';
                        }
                    }
        ...
        if tetelek:
            for t in tetelek:
                menny = str(t.get("mennyiseg", "")).strip()
                egys  = str(t.get("egyseg", "")).strip()
                t["mennyiseg"] = f"{menny} {egys}".strip() if menny else ""
        ...
        # Fizetési feltétel + érvényesség: ismert Innonest mezőnevek alapján
        fizetes_ervenyes = await page.evaluate("""
            () => {
                const result = { fizetesi: '', ervenyes: '' };

                const payEl = document.querySelector(
                    'select[name="paymentMethod"], select[name*="payment" i], input[name="paymentMethod"]'
                );
                if (payEl) {
                    if (payEl.tagName === 'SELECT') {
                        const opt = payEl.selectedOptions && payEl.selectedOptions[0];
                        result.fizetesi = opt ? opt.text.trim() : (payEl.value || '').trim();
                    } else {
                        result.fizetesi = (payEl.value || '').trim();
                    }
                }

                const expEl = document.querySelector(
                    'input[name="expiration"], input[name*="expir" i], input[name*="ervenyes" i]'
                );
                if (expEl) {
                    result.ervenyes = (expEl.value || '').trim();
                }
        ...
        if fizetes_ervenyes.get("fizetesi"):
            eredmeny["fizetesi_feltetelek"] = fizetes_ervenyes["fizetesi"]
        else:
            fizetes_m = re.search(r"[Ff]izet[eé]si feltétel[ek]*[:\s]*([^\n]{5,80})", page_text)
            if fizetes_m:
                eredmeny["fizetesi_feltetelek"] = fizetes_m.group(1).strip()

        if fizetes_ervenyes.get("ervenyes"):
            eredmeny["ervenyes_ig"] = fizetes_ervenyes["ervenyes"]
        else:
        ...
        log.info(
            ...
            f"fizetés={eredmeny['fizetesi_feltetelek']}, érvényes={eredmeny['ervenyes_ig']}"
        )
