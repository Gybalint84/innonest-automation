...
async def fill_input(page, locator, value):
    """
    Megbízható mezőkitöltés headless módban is:
    scroll → click → Ctrl+A → Delete → type karakter-ként
    """
    await locator.scroll_into_view_if_needed()
    await locator.click()
    await page.wait_for_timeout(150)
    # Háromszor próbál – néha az első click nem veszi át a fókuszt
    for _ in range(3):
    ...
    if str(value) not in actual:
        ...
        await page.evaluate(
            ...
            {"el": await locator.element_handle(), "val": str(value)}
        )
    print(f"     ✅ '{value}'")


async def find_and_fill(page, js_getter, value, label):
    """
    JS-sel megkeresi az elemet, majd fill_input-tal tölti ki.
    Headless-barát: nem triple_click, hanem Ctrl+A+Delete+type.
    """
    try:
        ...
        if el:
            loc = page.locator(
                f'input[placeholder="Egységár"]'
                if "Egar" in label else
                f'[placeholder]'
            )
            # Inkább az ElementHandle-t használjuk közvetlen Playwright akcióval
            ...
            print(f"     ⚠️  {label}: JS null-t adott vissza")
        await h.dispose()
    except Exception as e:
        print(f"     ❌ {label}: {e}")
...
async def run_automation(payload: dict):
    ...
    async with async_playwright() as p:
        ...
        # Név: Tab-bal lépünk ki (nem Escape – az törli a tartalmat)
        nev_input = page.locator('input[placeholder="Ügyfél neve"]').first
        await nev_input.scroll_into_view_if_needed()
        await nev_input.click()
        await page.wait_for_timeout(300)
        await page.keyboard.press("Control+a")
        await page.keyboard.press("Delete")
        await page.wait_for_timeout(150)
        nev_ertek = ugyfel.get("nev", "1") or "1"
        await page.keyboard.type(nev_ertek, delay=60)
        await page.wait_for_timeout(500)
        await page.keyboard.press("Tab")
        await page.wait_for_timeout(400)
        # Ha üres maradt, újra
        nev_val = await nev_input.input_value()
        if not nev_val.strip():
            ...
            await page.keyboard.press("Tab")
            await page.wait_for_timeout(300)
        print(f"   Név: '{await nev_input.input_value()}'")

        for sel, key, default in [
            ('input[placeholder="Adószám"]',            "adoszam",        ""),
            ('input[placeholder="Irányítószám"]',       "iranyitoszam",   ""),
            ('input[placeholder="Település"]',          "telepules",      ""),
            ('input[placeholder="Utca"]',               "utca",           ""),
            ('input[placeholder="Kapcsolattartó neve"]',"kapcsolattarto", ""),
        ]:
            ertek = ugyfel.get(key, default)
            if ertek:
                await fill_input(page, page.locator(sel).first, ertek)

        # ── Árajánlat tárgya ──
        print(f"📄 Árajánlat tárgya: '{targya}'")
        await fill_input(page, page.locator('input[placeholder="Árajánlat tárgya"]').first, targya)

        # ── Tételek ──
        print(f"📋 {len(items)} tétel...")
        for i, item in enumerate(items):
            print(f"   [{i+1}/{len(items)}] {item['megnevezes'][:45]}")
            ...
            await fill_input(page, megnev, item["megnevezes"])
            ...
            await find_and_fill(page, JS_MENNY, item["mennyiseg"], "Mennyiség")
            await find_and_fill(page, JS_EGYS,  item["egyseg"],   "Egység")
            await find_and_fill(page, JS_EGAR,  item["egysegar"], "Egységár")
