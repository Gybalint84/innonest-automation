...
async def get_megrendelt_tetelek(page) -> list[dict]:
    ...
    A cég nevét és az árajánlat tárgyát közvetlenül a listából olvassa ki.
    """
    log.info("Megrendelőlapok ellenőrzése...")
    await page.goto("https://app.innonest.hu/ordersheets", wait_until="networkidle")
    await page.wait_for_timeout(2000)

    # Az összes adatot JS-ből nyerjük ki egyszerre
    tetelek_raw = await page.evaluate("""
        () => {
            const eredmenyek = [];
            // Minden táblázatsor / listaelem
            const sorok = document.querySelectorAll("tr, .list-item, [class*='row']");

            sorok.forEach(sor => {
                const szoveg = sor.innerText || "";

                // BID szám keresése
                const bidMatch = szoveg.match(/BID-\d{4}-\d+/);
                if (!bidMatch) return;

                // Csak Megrendelt státuszú sorok
                if (!szoveg.toLowerCase().includes("megrendelt")) return;

                const bid = bidMatch[0];

                // data-id attribútum
                const rowId = sor.getAttribute("data-id") || bid;

                // Cég neve: általában a második sorban jelenik meg a listában
                const sorok_szoveg = szoveg.split("\n").map(s => s.trim()).filter(s => s);
                let cegnev = "";
                let targya = "";

                // A megnevezés (árajánlat tárgya) az első érdemi sor
                // A cég neve általában utána következik
                for (let i = 0; i < sorok_szoveg.length; i++) {
                    const sor_szoveg = sorok_szoveg[i];
                    // Kihagyjuk a BID számot, az összeget, a dátumot, az állapotot
                    if (sor_szoveg.match(/BID-/)) continue;
                    if (sor_szoveg.match(/^\d{4}-\d{2}-\d{2}/)) continue;
                    if (sor_szoveg.match(/HUF|EUR|USD/)) continue;
                    if (sor_szoveg.toLowerCase().match(/megrendelt|piszkozat|elküldve/)) continue;
                    if (sor_szoveg.match(/^\d{1,3}([ .]\d{3})*$/)) continue;
                ...
                // Pénznem keresése
                const penznemMatch = szoveg.match(/\b(HUF|EUR|USD|GBP|CHF)\b/);
                const penznem = penznemMatch ? penznemMatch[1] : "HUF";

                // Nettó összeg: számok amelyek nagy értékek (valószínűleg az összeg)
                const szamok = [...szoveg.matchAll(/(\d{1,3}(?:[ ]\d{3})+|\d{5,})/g)];
                const netto = szamok.length > 0 ? szamok[0][0].replace(/\s/g, "") : "";

                // Az árajánlat detail linkje (nem PDF!)
                const linkek = sor.querySelectorAll("a[href]");
                let detailLink = null;
                linkek.forEach(link => {
                    const href = link.getAttribute("href") || "";
                    // PDF linkeket kihagyjuk
                    if (!href.includes("pdf") && !href.includes("print") && href.includes("/")) {
    ...
    for t in egyedi:
        log.info(f"  → {t['bid']}: cég='{t['cegnev']}', tárgy='{t['targya'][:40]}', "
                 f"pénznem={t['penznem']}, nettó={t['netto']}")
    return egyedi
...
async def get_arajanlat_reszletek(page, bid: str) -> dict:
    ...
    try:
        ...
        if await bid_elem.count() > 0:
            ...
            adatok = await page.evaluate("""
                () => {
                    const text = document.body ? document.body.innerText : "";

                    const penznemMatch = text.match(/\b(HUF|EUR|USD|GBP|CHF)\b/);
                    const penznem = penznemMatch ? penznemMatch[1] : "HUF";

                    // Nettó összeg keresése
                    const nettoMatch = text.match(/[Nn]ett[oó][^\d]*(\d[\d\s\.\,]*)/);
                    const netto = nettoMatch ? nettoMatch[1].replace(/[\s]/g, "").trim() : "";

                    return { penznem, netto };
                }
            """)
            reszletek["penznem"] = adatok.get("penznem", "HUF")
            reszletek["netto"]   = adatok.get("netto", "")
            log.info(f"Árajánlat adatok ({bid}): pénznem={reszletek['penznem']}, nettó={reszletek['netto']}")
        else:
