...
# Egyszerre csak 1 Playwright böngésző futhat.
# Ha a figyelő fut, az árajánlat kérés vár és fordítva – így nem fogy el az erőforrás.
...
async def run_automation(payload: dict):
    ...
    log.info("Árajánlat: böngésző indítás várakozás...")
    acquired = _browser_lock.acquire(timeout=240)
    if not acquired:
        raise Exception("Böngésző foglalt (timeout), próbáld újra!")
    ...
    try:
     async with async_playwright() as p:
        ...
        log.info("Árajánlat: böngésző felszabadítva.")
...
async def check_megrendelesek():
    ...
    if not _browser_lock.acquire(timeout=60):
        log.warning("Figyelő: böngésző foglalt, kihagyom ezt a kört.")
        return
    log.info("Figyelő: böngésző indítás...")
    try:
     async with async_playwright() as pw:
        ...
        _browser_lock.release()
        log.info("Figyelő: böngésző felszabadítva.")
