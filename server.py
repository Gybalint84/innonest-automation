...
async def get_megrendelt_tetelek(page) -> list[dict]:
    ...
    for sor in sorok_raw:
        ...
        # BID szám keresése – szigorúan csak BID-YYYY-NN formátum
        ...
        if has_bid:
            ...
            # Ha nincs BID szám, a megrendelőlap sorszámát használjuk (pl. "2026-60")
        ...
        # Pénznem + nettó + árfolyam kinyerése SORONKÉNT
        # Minden sornál két összeg látható: nettó (eredeti pénznem) + HUF egyenérték
        # Árfolyam = HUF összeg / nettó (ha EUR/USD), HUF esetén = 1
        penznem = "HUF"
        netto = ""
        arfolyam = "1"
        osszes_osszeg = []  # [(szam, penznem)] párok a sorból
        ...
        if osszes_osszeg:
            # Az első érvényes összeg a nettó (eredeti pénznemben)
            ...
            if penznem == "HUF":
                ...
                # Keresünk egy HUF összeget a többi között az árfolyam kiszámításához
        ...
        log.info(f"  → {bid}: cég='{cegnev}', tárgy='{targya[:40]}', pénznem={penznem}, nettó={netto}, árfolyam={arfolyam}")
        tetelek.append({
            "row_id":   row_id,
            "bid":      bid,
            "has_bid":  has_bid,
...
async def get_arajanlat_tetelek(page, bid: str) -> dict:
    """
    1. Megnyitja a bids listát
    2. Megkeresi a BID számhoz tartozó sort
    3. Kinyeri az árajánlat belső ID-ját a PDF linkből (worksheets_pdf/open/ID)
    4. Megnyitja a szerkesztő oldalt: /bids/change/ID
    5. Kinyeri a tételeket, nettó összeget, fizetési feltételt, érvényességet
    """
    eredmeny = {
        "tetelek": [],
        "netto_osszeg": "",
    ...
    try:
        ...
        # ── 1. BID sor megkeresése és belső ID kinyerése ──
        # A PDF link formátuma: /worksheets_pdf/open/10751882.html
        bid_link = await page.evaluate(f"""
            () => {{
                ...
                // Fallback: keressük a BID szöveget tartalmazó sort
        ...
        log.info(f"[TETELEK] PDF link megtalálva: {bid_link}")

        # ── 2. Belső ID kinyerése az URL-ből ──
        id_match = re.search(r"/worksheets_pdf/open/(\d+)", bid_link)
        if not id_match:
            # Próba: /bids/change/ID formátum
        ...
        log.info(f"[TETELEK] Szerkesztő URL: {szerkeszto_url} ({bid})")

        # ── 3. Szerkesztő oldal megnyitása ──
        await page.goto(szerkeszto_url, wait_until="networkidle")
        await page.wait_for_timeout(2000)

        # ── 4. Tételek kinyerése a szerkesztőből ──
        # A képen látott struktúra alapján:
        # - Tétel megnevezése: input mező értéke (első oszlop)
        # - Mennyiség + egység: következő inputok
        # - Nettó egységár: input
        # - Nettó összesen: szöveg (nem input)
        tetelek = await page.evaluate("""
            () => {
                const tetelek = [];
                let sorszam = 1;

                // Keresés: minden tételsor - input mezők alapján
                // A szerkesztőben minden tétel egy div/tr blokkban van
                // Az első input az a tétel neve
                const tetelBlokkok = document.querySelectorAll(
                    'div.form-row, div.item-row, tr.item-line, ' +
                    '[class*="tetel-sor"], [class*="item-row"], ' +
                    'div.sortable-item, div.line-item'
                );

                if (tetelBlokkok.length > 0) {
                    tetelBlokkok.forEach(blokk => {
                        const inputok = blokk.querySelectorAll('input[type="text"], input:not([type])');
                        if (inputok.length >= 2) {
                            const nev = inputok[0] ? inputok[0].value.trim() : '';
                            if (nev && nev.length > 1) {
                                // Mennyiség és egység
                                const menny = inputok[1] ? inputok[1].value.trim() : '';
                                const egys  = inputok[2] ? inputok[2].value.trim() : '';
                                // Egységár (általában 4. input)
                                const ear   = inputok[3] ? inputok[3].value.trim() : '';

                                // Összesen: keressük a sorban a nettó összesen szöveget
                                const szamok = blokk.querySelectorAll(
                                    '[class*="total"], [class*="összesen"], [class*="osszesen"], ' +
                                    'td:last-child, span.amount, span.netto'
                                );
                                let osszesen = '';
                                szamok.forEach(s => {
                                    const val = s.innerText.trim();
                                    if (/[0-9]{3,}/.test(val) && val.length < 20) {
                                        osszesen = val;
                                    }
                                });
                ...
                // Ha az első módszer nem hozott eredményt, próbáljuk táblázatként
                if (tetelek.length === 0) {
                    const sorok = document.querySelectorAll('table tbody tr');
                    sorok.forEach(tr => {
                        const inputok = tr.querySelectorAll('input');
                        const cellaTextok = Array.from(tr.querySelectorAll('td'))
                            .map(td => td.innerText.trim())
                            .filter(Boolean);

                        if (inputok.length >= 1 && inputok[0].value.trim().length > 1) {
                            const nev = inputok[0].value.trim();
                            const menny = inputok[1] ? inputok[1].value.trim() : '';
                            const egys  = inputok[2] ? inputok[2].value.trim() : '';
                            const ear   = inputok[3] ? inputok[3].value.trim() : '';
                            const osszesen = cellaTextok.find(t => /[0-9 .,]{5,}/.test(t)) || '';
        ...
        log.info(f"[TETELEK] Kinyert tételek száma: {len(tetelek)} ({bid})")
        if tetelek:
            eredmeny["tetelek"] = tetelek

        # ── 5. Oldalszöveg a nettó, fizetési feltétel, érvényesség kinyeréséhez ──
        ...
        # Nettó végösszeg — keressük a legnagyobb számot a "nettó" közelében
        netto_m = re.search(r"[Nn]ett[oó][^\n]{0,30}?([0-9][0-9 ]{3,}[0-9])", page_text)
        if netto_m:
            netto_raw = netto_m.group(1).strip().replace(" ", "")
            try:
                eredmeny["netto_osszeg"] = f"{int(float(netto_raw)):,}".replace(",", " ")
            except Exception:
                eredmeny["netto_osszeg"] = netto_raw

        # Fizetési feltétel
        fizetes_m = re.search(
            r"[Ff]izet[eé]si feltétel[ek]*[:\s]*([^\n]{5,80})", page_text
        )
        if fizetes_m:
        ...
        log.info(
            f"[TETELEK] Kész ({bid}): "
            f"tételek={len(eredmeny['tetelek'])}, "
            f"nettó={eredmeny['netto_osszeg']}, "
            f"pénznem={eredmeny['penznem']}, "
            f"fizetés={eredmeny['fizetesi_feltetelek']}, "
            f"érvényes={eredmeny['ervenyes_ig']}"
        )
...
async def _innonest_adatok_leker_async(bid: str) -> dict:
    """
    Önálló Playwright session az Innonest tételek lekéréséhez.
    A pipedrive_addon.py hívja meg run_in_loop()-on keresztül.
    """
...
def innonest_adatok_leker(bid: str) -> dict:
    ...
    Visszaad egy dict-et: tetelek, netto_osszeg, penznem,
