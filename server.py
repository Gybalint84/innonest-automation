...
async def get_arajanlat_tetelek(page, bid: str) -> dict:
    ...
    try:
        ...
        # Struktúra: tbody.items-box > tr.items (kihagyva tr[data-id="0"] fejléc)
        # Mezők: input[name^="productsName"], input[name^="productsQty"],
        #        input[name^="productsPrice"], nettó összesen: td.right szöveg
        tetelek = await page.evaluate("""
            () => {
                ...
                sorok.forEach(tr => {
                    ...
                    // Egység: a "smaller red" div szövege pl. "365 M2" → "m2"
                    let egys = '';
                    ...
                    // Nettó összesen: td.right szövegek közül az utolsó számszerű
                    const tdok = Array.from(tr.querySelectorAll('td.right'));
                    let osszesen = '';
                    for (let i = tdok.length - 1; i >= 0; i--) {
                        const szov = tdok[i].innerText.replace(/\\s+/g, ' ').trim();
                        if (/^[0-9]/.test(szov) && /[0-9]{3}/.test(szov)) {
                            osszesen = szov;
                            break;
                        }
                    }
        ...
        # ── 5. Nettó végösszeg kinyerése a táblázat láblécéből ──
        netto_js = await page.evaluate("""
            () => {
                // Az összesítő sor általában tfoot-ban vagy az items-box után
                // Keresés: "Nettó összesen" felirat melletti szám
                const osszes = document.querySelectorAll(
                    'tfoot td, tr.total td, tr.summary td, ' +
                    '[class*="total"] td, [class*="osszesen"], ' +
                    'td.netto-total, span.grand-total'
                );
                for (const el of osszes) {
                    const szov = el.innerText.replace(/\\s+/g, '').trim();
                    if (/^[0-9]{4,}/.test(szov)) return szov;
                }

                // Fallback: keressük a legnagyobb számot az oldalon
                // ami valószínűleg a végösszeg
                const allText = document.body.innerText;
                const netto = allText.match(/[Nn]ett[oó][^\\n]{0,30}?([0-9][0-9 ]{4,}[0-9])/);
                if (netto) return netto[1].replace(/\\s+/g, '');
                return '';
            }
        """)
        ...
        # ── 6. Oldalszöveg a fizetési feltétel és érvényesség kinyeréséhez ──
