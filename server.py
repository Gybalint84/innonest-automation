...
async def get_megrendelt_tetelek(page) -> list[dict]:
    ...
    for sor in sorok_raw:
        ...
        # Pénznem + nettó kinyerése SORONKÉNT
        # A sor szövegét sorokra bontjuk, és minden sorban keresünk szám+pénznem párt
        # Így a kis "0" számok (ikonszámlálók) nem keverednek össze az összeggel
        penznem = "HUF"
        netto = ""
        for sor_r in szoveg.splitlines():
            sor_r = sor_r.strip()
            # Keressük a "SZÁM PÉNZNEM" mintát (pl. "16 068 384 HUF" vagy "3 239 EUR")
            penz_m = re.search(r"([0-9][0-9 ]{3,}[0-9])\s*(HUF|EUR|USD|GBP|CHF)", sor_r)
            if penz_m:
                netto_jelolt = penz_m.group(1).replace(" ", "")
                # Csak akkor fogadjuk el ha legalább 4 számjegy (ezres nagyságrendű összeg)
                if len(netto_jelolt) >= 4:
                    netto = netto_jelolt
