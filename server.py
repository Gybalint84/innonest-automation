...
async def upload_csatolmany(page, csatolmany: dict, bid_szam: str):
    """
    Feltölti az xlsx fájlt az árajánlat csatolmányaiba.
    Két módszert próbál:
    1. Az aktuális oldalon lévő csatolmány gomb
    2. Visszalép a listára és ott kattint a gémkapocs ikonra
    """
    import tempfile, os
    ...
    try:
        ...
        if await paperclip.count() > 0:
            ...
            # ── 2. kísérlet: badge-jelű ikonok a listából ──
        ...
        # ── Csatolmányok dialóg: fájl input keresése ──
        # Playwright set_input_files() hidden inputon is működik
        await page.wait_for_timeout(500)
        file_input = page.locator('input[type="file"]').first
        if await file_input.count() > 0:
            await file_input.set_input_files(tmp_path)
            await page.wait_for_timeout(3000)  # feltöltés befejezéséig
            print(f"   ✅ Fájl feltöltve: {fajl_nev}")
        else:
            # Ha nincs direktbe input, kattintsunk a feltöltési zónára
...
async def run_automation(payload: dict):
    ...
    async with async_playwright() as p:
        ...
        # ── BID szám kinyerése a mentés utáni oldalból ──
