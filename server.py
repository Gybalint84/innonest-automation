...
async def get_megrendelt_tetelek(page) -> list[dict]:
    ...
    for sor in sor_elemek:
        try:
            ...
            if "megrendelt" not in sor_szoveg.lower():
                continue
            row_id = await sor.get_attribute("data-id") or bid

            # Link kinyerése a részletoldalhoz
            link_elem = await sor.query_selector("a[href]")
            link = await link_elem.get_attribute("href") if link_elem else None

            tetelek.append({"row_id": row_id, "bid": bid, "link": link})
        except Exception:
            continue
...
async def get_megrendeles_reszletek(page, link: str, bid: str) -> dict:
    """
    Megnyitja a megrendelőlap részletoldalát és kinyeri:
    - cég neve
    - árajánlat tárgya
    - pénznem
    - nettó összérték
    """
    reszletek = {"cegnev": "", "targya": "", "penznem": "", "netto": ""}

    try:
        url = f"https://app.innonest.hu{link}" if link and link.startswith("/") else link
        if not url:
            # Ha nincs link, próbáljuk az árajánlat oldalán megkeresni a BID-et
            await page.goto("https://app.innonest.hu/bids", wait_until="networkidle")
            await page.wait_for_timeout(1500)
        ...
        log.info(f"Részletek oldal: {page.url}")

        # Szöveg kinyerése az oldalból JavaScript-tel
        adatok = await page.evaluate("""
            () => {
                const text = document.body ? document.body.innerText : "";
                const lines = text.split("\n").map(l => l.trim()).filter(l => l);

                // Segédfüggvény: érték keresése kulcsszó után
                function findAfter(keyword) {
                    for (let i = 0; i < lines.length; i++) {
                        if (lines[i].toLowerCase().includes(keyword.toLowerCase())) {
                            // A következő nem üres sor az érték
                ...
                // Pénznem keresése (HUF, EUR, USD stb.)
                const penznemMatch = text.match(/\b(HUF|EUR|USD|GBP|CHF)\b/);
                const penznem = penznemMatch ? penznemMatch[1] : "";

                // Nettó összeg keresése (számok formátuma)
                const nettoMatch = text.match(/[Nn]ett[oó][:\s]+([\d\s\.\,]+)/);
                const netto = nettoMatch ? nettoMatch[1].replace(/\s/g, "").trim() : "";

                return {
                    cegnev:  findAfter("Ügyfél neve") || findAfter("Ügyfél") || findAfter("Vevő"),
                    targya:  findAfter("Árajánlat tárgya") || findAfter("Tárgy") || findAfter("Megnevezés"),
                    penznem: penznem,
                    netto:   netto,
        ...
        reszletek["cegnev"]  = adatok.get("cegnev", "")
        reszletek["targya"]  = adatok.get("targya", "")
        reszletek["penznem"] = adatok.get("penznem", "")
        reszletek["netto"]   = adatok.get("netto", "")

        log.info(f"Kinyert adatok ({bid}): cég={reszletek['cegnev']}, tárgy={reszletek['targya']}, "
                 f"pénznem={reszletek['penznem']}, nettó={reszletek['netto']}")

    except Exception as e:
        log.error(f"Részletek kinyerési hiba ({bid}): {e}")
...
async def check_megrendelesek():
    ...
    async with async_playwright() as pw:
        ...
        for tetel in tetelek:
            ...
            # Részletek kinyerése az Innonestből
            reszletek = await get_megrendeles_reszletek(page, link, bid)
            ...
            try:
                response = requests.post(
                    ...
                    json={
                        ...
                        "bid":     bid,
                        "cegnev":  reszletek["cegnev"],
                        "targya":  reszletek["targya"],
                        "penznem": reszletek["penznem"],
                        "netto":   reszletek["netto"],
                    },
                    timeout=30
                )
                result = response.json()
