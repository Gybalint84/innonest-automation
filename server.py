...
async def get_arajanlat_tetelek(page, bid: str) -> dict:
    """
    Megnyitja az árajánlat oldalát BID alapján és kinyeri:
    - tételsorok (megnevezés, mennyiség, egységár, összesen)
    - nettó végösszeg
    - fizetési feltétel
    - érvényességi dátum
    - pénznem
    ...
    try:
        ...
        bid_elem = page.locator(f"text={bid}").first
        if await bid_elem.count() == 0:
            log.warning(f"[TETELEK] Nem találtam az árajánlatot: {bid}")
            return eredmeny

        await bid_elem.click()
        await page.wait_for_load_state("networkidle")
        await page.wait_for_timeout(2000)
        log.info(f"[TETELEK] Árajánlat oldal megnyílt: {page.url}")
        ...
        # ── Nettó végösszeg ──
        netto_m = re.search(r"[Nn]ett[oó].{0,20}?([0-9][0-9 .,]{3,})", page_text)
        if netto_m:
            netto_raw = netto_m.group(1).strip().replace(" ", "").replace(",", ".")
            try:
        ...
        ervenyes_m = re.search(
            r"[Éé]rv[eé]nyes[sé]*[eig]*[:\s]*([^\n]{5,40})", page_text
        )
        if ervenyes_m:
            eredmeny["ervenyes_ig"] = ervenyes_m.group(1).strip()

        # ── Tételsorok kinyerése táblázatból ──
        tetelek = await page.evaluate("""
            () => {
                const rows = [];
                // Keresés: táblázat sorok ahol van megnevezés + ár
                const tableRows = document.querySelectorAll('table tbody tr, [class*="item"] [class*="row"], [class*="tetel"]');
                let sorszam = 1;
                tableRows.forEach(tr => {
                    const cells = tr.querySelectorAll('td, [class*="cell"], [class*="col"]');
                    if (cells.length >= 3) {
                        const szovegek = Array.from(cells).map(c => c.innerText.trim()).filter(Boolean);
                        // Szűrés: kell legalább egy szám (ár) és egy szöveges rész (megnevezés)
                        const vanSzam = szovegek.some(s => /[0-9]{3,}/.test(s));
                        const vanSzoveg = szovegek.some(s => s.length > 3 && !/^[0-9.,\\s]+$/.test(s));
                        if (vanSzam && vanSzoveg) {
                            rows.push({
                                sorszam: sorszam++,
                                megnevezes: szovegek[0] || '',
                                mennyiseg:  szovegek[1] || '',
                                egysegar:   szovegek[2] || '',
                                osszesen:   szovegek[szovegek.length - 1] || ''
                            });
                        }
                    }
                });
        ...
        if tetelek:
            ...
            # Fallback: szöveges kinyerés soronként
            log.warning(f"[TETELEK] Táblázatos kinyerés sikertelen, szöveges fallback ({bid})")
            sorok = page_text.split("\n")
            tetelek_fb = []
            sorszam = 1
            for sor in sorok:
                sor = sor.strip()
                if len(sor) > 5 and re.search(r"[0-9]{3,}", sor) and len(sor) < 200:
                    reszek = sor.split()
                    if len(reszek) >= 2:
                        tetelek_fb.append({
                            "sorszam": sorszam,
                            "megnevezes": " ".join(reszek[:-1]),
                            "mennyiseg": "",
                            "egysegar": "",
                            "osszesen": reszek[-1]
                        })
                        ...
                        if sorszam > 20:
                            break
            eredmeny["tetelek"] = tetelek_fb

        log.info(f"[TETELEK] Eredmény ({bid}): nettó={eredmeny['netto_osszeg']}, "
