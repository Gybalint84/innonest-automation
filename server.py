...
async def get_megrendelt_tetelek(page) -> list[dict]:
    ...
    for sor in sorok_raw:
        ...
        for s in sorok_lista:
            # BID számot kihagyjuk
            if re.search(r"BID-", s): continue
            # Dátumot kihagyjuk (YYYY-MM-DD)
            if re.search(r"^\d{4}-\d{2}-\d{2}", s): continue
            # Megrendelőlap számát kihagyjuk (pl. "2026-60")
            ...
            # Csak számokat tartalmazó sorokat kihagyjuk
            if re.match(r"^[\d\s\.,]+$", s): continue
            # Rövid badge-eket kihagyjuk (KIV, BEJ stb. - max 5 karakter)
        ...
        # Pénznem keresése
        penznem_match = re.search(r"(HUF|EUR|USD|GBP|CHF)", szoveg)
        penznem = penznem_match.group(1) if penznem_match else "HUF"

        # Nettó összeg keresése
        szamok = re.findall(r"\d{1,3}(?:[\s]\d{3})+|\d{5,}", szoveg)
        netto = szamok[0].replace(" ", "") if szamok else ""
