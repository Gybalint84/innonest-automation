...
async def _add_cim(page, szoveg: str) -> bool:
    """A 'Új tétel hozzáadása' split-gomb legördülőjéből 'Új cím hozzáadása',
    majd KIZÁRÓLAG a frissen megjelenő (új) mező kitöltése.

    Fontos: a meglévő mezőket (pl. 'Szállítási cím') előbb megjelöljük, és
    csak a JELÖLETLEN, újonnan létrejött mezőbe írunk — így soha nem kerül a
    szöveg rossz helyre. Ha nem jelenik meg új mező, inkább kihagyjuk (a
    megrendelés így is elmentődik), és a logból élesben igazítható a menüpont.
    A tételek BEÍRÁSA ELŐTT kell hívni."""

    # 0) A JELENLEGI mezők megjelölése — utána a jelöletlen = az új cím-mező
    try:
        await page.evaluate(
    ...
    # 1) A legördítő (caret) megnyitása — a kék gomb melletti kis nyíl
    toggle = None
    uj = page.locator('button:has-text("Új tétel hozzáadása")').first
    if await uj.count() > 0:
        cand = uj.locator(
            'xpath=ancestor::*[contains(@class,"btn-group")][1]//button[contains(@class,"dropdown-toggle")]'
        ).first
        if await cand.count() > 0:
            toggle = cand
        else:
            sib = uj.locator('xpath=following-sibling::button[1]')
            if await sib.count() > 0:
                toggle = sib
    if toggle is None:
        alt = page.locator('button.dropdown-toggle').first
        if await alt.count() > 0:
            toggle = alt
    if toggle is None:
        log.warning("[BESZERMEGREND] Nem találtam a tétel-legördítő gombot — cím kihagyva.")
        return False
    try:
        ...
        log.warning(f"[BESZERMEGREND] Legördítő megnyitás hiba: {e}")

    # 2) 'Új cím hozzáadása' menüelem — lehetőleg látható
    cim_item = page.locator(
        'a:has-text("Új cím hozzáadása"), button:has-text("Új cím hozzáadása"), '
        'li:has-text("Új cím hozzáadása"), span:has-text("Új cím hozzáadása"), '
        'div:has-text("Új cím hozzáadása")'
    ).first
    if not await cim_item.count():
        log.warning("[BESZERMEGREND] Nem találtam az 'Új cím hozzáadása' menüpontot — cím kihagyva.")
        return False
    try:
        await cim_item.scroll_into_view_if_needed()
        await cim_item.click()
    except Exception as e:
        log.warning(f"[BESZERMEGREND] 'Új cím hozzáadása' kattintás hiba: {e} — cím kihagyva.")
        return False
    await page.wait_for_timeout(1000)

    # 3) Az ÚJONNAN megjelent (jelöletlen) mező kitöltése — SOHA nem a meglévőkbe
    new_field = page.locator(
        'input:not([data-precim]):visible, textarea:not([data-precim]):visible, '
        '[contenteditable="true"]:not([data-precim]):visible'
    ).first
    if await new_field.count() == 0:
        log.warning("[BESZERMEGREND] 'Új cím hozzáadása' után nem jelent meg új mező — "
                    "a szöveget NEM írom máshova. Élesben ellenőrizni kell a menüpontot.")
        return False
    try:
        await new_field.scroll_into_view_if_needed()
        await new_field.click()
        tag = await new_field.evaluate("el => el.tagName.toLowerCase()")
        if tag in ("input", "textarea"):
            await new_field.fill(str(szoveg))
            await new_field.evaluate(
        ...
        log.info(f"[BESZERMEGREND] Cím beírva az új mezőbe: {szoveg}")
        return True
    except Exception as e:
...
async def run_automation(payload: dict):
    ...
    async with async_playwright() as p:
        ...
        # 3b. Cím a tételek elé: "Kérem a számlán feltüntetni: {BID}"
