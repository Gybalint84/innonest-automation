...
async def _fill_last(page, sel: str, value: str) -> bool:
    """A szelektorra illeszkedő UTOLSÓ (frissen hozzáadott) mező kitöltése."""
...
async def _add_cim(page, szoveg: str) -> bool:
    ...
    majd a megjelenő cím-mező kitöltése a megadott szöveggel.
    ...
    if toggle is None:
        ...
        if await alt.count() > 0:
            toggle = alt
    if toggle is None:
        log.warning("[BESZERMEGREND] Nem találtam a tétel-legördítő gombot.")
        return False
    try:
    ...
    if not await cim_item.count():
        log.warning("[BESZERMEGREND] Nem találtam az 'Új cím hozzáadása' menüelemet.")
        return False
    await cim_item.scroll_into_view_if_needed()
    await cim_item.click()
    await page.wait_for_timeout(900)

    # 3) A frissen megjelent cím-mező kitöltése (több lehetséges szelektor)
    for sel in [
        'input[placeholder="Cím"]', 'input[placeholder*="ím"]',
        'textarea[placeholder="Cím"]', 'textarea[placeholder*="ím"]',
        'input.title', 'input.cim', '.title-row input', '.cim input',
    ]:
        if await _fill_last(page, sel, szoveg):
            log.info(f"[BESZERMEGREND] Cím beírva ('{sel}'): {szoveg}")
            return True
    log.warning("[BESZERMEGREND] Cím hozzáadva, de a cím-mezőt nem találtam a kitöltéshez — "
                "élesben ellenőrizni kell a szelektort.")
    return False
