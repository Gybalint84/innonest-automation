...
ANYAG_FEJLEC = "A kivitelezéshez a megrendelő az alábbi alapanyagokat biztosítja:"


def _elso_tetel_fejlec(payload: dict) -> str:
    """Az ELSŐ tétel szövegboxának bevezetője: kivitelezés helyszíne + a projekt
    kezdés/befejezés dátuma (a Pipedrive projektből)."""
    sorok = []
    helyszin = str(payload.get("kivitelezesHelyszin", "") or "").strip()
    start    = str(payload.get("projektStart", "") or "").strip()
    veg      = str(payload.get("projektEnd", "") or "").strip()
    if helyszin:
        sorok.append(f"Kivitelezés helyszíne: {helyszin}")
    if start:
        sorok.append(f"Kezdés: {start}")
    if veg:
        sorok.append(f"Befejezés: {veg}")
    return "\n".join(sorok)


def _combine_megjegyzes(*reszek: str) -> str:
    """Nem üres szövegrészek összefűzése üres sorral elválasztva."""
    return "\n\n".join(r for r in reszek if r and r.strip())


def _anyag_megjegyzes(anyagok: list) -> str:
    """A tétel anyaglistája a megjegyzés-szövegboxba: egy bevezető mondat, majd
    soronként 'Név – menny egység'. Ha nincs anyag, üres marad (fejléc nélkül)."""
    sorok = []
    for a in (anyagok or []):
...
async def _add_cim(page, szoveg: str) -> bool:
    """'Új cím hozzáadása' (a.addNewItemTitle) → az új cím-sor kitöltése, majd a
    cím-sor a tételek ELÉ mozgatása (a tbody.items-box fejléce után).

    A pontos szelektorok az élő Innonest /acquisition/add oldal alapján:
      - a menüpont az <a class="addNewItemTitle"> (a split-gomb legördülőjében),
      - az új cím-mező: input[placeholder^="Cím megadása"] (name=productsName_N),
      - a sorok a tbody.items-box (ui-sortable) <tr> gyermekei; a mentés a DOM
        sorrendjét követi, ezért a cím-sort a fejléc után, az első tétel elé tesszük.

    A TÉTELEK KITÖLTÉSE UTÁN, a MENTÉS ELŐTT kell hívni."""

    # A meglévő 'Cím megadása' inputok megjelölése, hogy az újat biztosan megkülönböztessük
    try:
        await page.evaluate(
            "() => document.querySelectorAll('input[placeholder^=\"Cím megadása\"]')"
            ".forEach(e => e.setAttribute('data-precim','1'))"
        )
    except Exception:
        pass

    # 1) 'Új cím hozzáadása' — közvetlen kattintás az anchorra (a legördülő nyitása
    #    nélkül is működik, mert a click handler magára az <a>-ra van kötve)
    clicked = False
    try:
        clicked = await page.evaluate(
            "() => { const a = document.querySelector('a.addNewItemTitle');"
            " if (!a) return false; a.click(); return true; }"
        )
    except Exception as e:
        log.warning(f"[BESZERMEGREND] cím anchor JS-kattintás hiba: {e}")
    if not clicked:
        # fallback: legördítő megnyitása + Playwright kattintás
        try:
            tgl = page.locator('button.dropdown-toggle').first
            if await tgl.count() > 0:
                await tgl.click()
                await page.wait_for_timeout(400)
            await page.locator('a.addNewItemTitle').first.click()
            clicked = True
        except Exception as e:
            log.warning(f"[BESZERMEGREND] 'Új cím hozzáadása' fallback kattintás hiba: {e}")
    if not clicked:
        log.warning("[BESZERMEGREND] Nem sikerült az 'Új cím hozzáadása' — cím kihagyva.")
        return False
    await page.wait_for_timeout(800)

    # 2) Az új (jelöletlen) cím-mező kitöltése
    field = page.locator('input[placeholder^="Cím megadása"]:not([data-precim])').first
    if await field.count() == 0:
        log.warning("[BESZERMEGREND] Nem jelent meg cím-mező a kattintás után — cím kihagyva.")
        return False
    try:
        await field.scroll_into_view_if_needed()
        await field.click()
        await field.fill(str(szoveg))
        await field.evaluate(
            "el => { el.dispatchEvent(new Event('input',{bubbles:true}));"
            " el.dispatchEvent(new Event('change',{bubbles:true})); }"
        )
    except Exception as e:
        log.warning(f"[BESZERMEGREND] cím-mező kitöltés hiba: {e}")
        return False

    # 3) A cím-sort a tételek ELÉ mozgatjuk (fejléc-sor után, első tétel elé)
    try:
        moved = await page.evaluate(
            """() => {
                const tbody = document.querySelector('tbody.items-box');
                if (!tbody) return 'no-tbody';
                const rows = [...tbody.querySelectorAll(':scope > tr')];
                const titleTr = rows.find(tr => tr.querySelector('input[placeholder^="Cím megadása"]'));
                const firstItemTr = rows.find(tr => tr.querySelector('input[name^="productsName_"]') && tr !== titleTr);
                if (titleTr && firstItemTr && titleTr !== firstItemTr) {
                    tbody.insertBefore(titleTr, firstItemTr);
                    return 'moved';
                }
                return 'nomove';
            }"""
        )
        log.info(f"[BESZERMEGREND] Cím beírva és a tételek elé mozgatva ({moved}): {szoveg}")
    except Exception as e:
        log.warning(f"[BESZERMEGREND] cím-sor mozgatás hiba: {e}")
    return True
...
async def run_automation(payload: dict):
    ...
    async with async_playwright() as p:
        ...
        # 4. Tételek — az első az induló sorba, a többihez "Új tétel hozzáadása".
        #    Az ELSŐ tétel szövegboxába a helyszín + kezdés/befejezés dátum is bekerül,
        #    az anyaglista elé.
        first = items[0]
        elso_megjegyzes = _combine_megjegyzes(
            _elso_tetel_fejlec(payload),
            _anyag_megjegyzes(first.get("anyagok")),
        )
        await fill_tetel(
            page, 1,
        ...
        # 4b. Cím a tételek ELÉ: "Kérem a számlán feltüntetni: {BID}"
        #     (a tételek után adjuk hozzá, majd a cím-sort a lista tetejére mozgatjuk)
        cim_szoveg = f"Kérem a számlán feltüntetni: {bid}" if bid else "Kérem a számlán feltüntetni:"
        try:
            await _add_cim(page, cim_szoveg)
        except Exception as e:
            log.warning(f"[BESZERMEGREND] Cím hozzáadás hiba (nem kritikus): {e}")
        await page.wait_for_timeout(300)
